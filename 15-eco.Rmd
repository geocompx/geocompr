...
```{r study-area-mongon, echo=FALSE, fig.cap="The Mt. Mongón study area. Figure taken from Muenchow, Schratz, and Brenning (2017).", out.width="60%", fig.scap="The Mt. Mongón study area."}
...
p1 <- xyplot(
  ...
  auto.key = list(
    x = 0.7, y = 0.9, text = c("unrotated", "rotated"),
    between = 0.5, cex = 0.9
  ),
  panel = function(x, y, ...) {
    # Plot the points
    ...
    panel.points(
      x = scores(nmds, display = "sites")[, 1],
      y = scores(nmds, display = "sites")[, 2],
      col = "salmon", pch = 16, cex = 0.6
    )
    panel.arrows(
      x0 = scores(nmds, display = "sites")[, 1],
...
points(scores(rotnmds, choices = 1:2, display = "sites"),
  col = "lightblue",
  pch = 16
)
...
#   tm_raster(
#     col.scale = tm_scale_continuous(values = rev(hcl.colors(99, "Grays"))),
#     col.legend = tm_legend_hide()
#   ) +
#   tm_shape(pred, is.main = TRUE) +
#   tm_raster(
#     col.scale = tm_scale_continuous(values = pal, midpoint = NA),
