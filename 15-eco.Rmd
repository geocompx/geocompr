...
The example, building on previous applied chapters, provides an opportunity to bring together and extend concepts presented in previous, to further enhance your skills at using R for geocomputation.

Fog oases, locally called *lomas*, are ecological systems found in the mountains along the coastal deserts of Peru and Chile.
...
Extreme conditions and remoteness provide the habitat for a unique ecosystem, including species endemic to the fog oases.
Despite the arid conditions and low levels of precipitation of around 30-50 mm per year on average, fog deposition increases the amount of water available to plants during austral winter, resuling in green southern-facing mountain slopes along the coastal strip of Peru. 
...
In it you will analyze the composition and the spatial distribution of vascular plants (plants which have woody or 'lignified tissue' such as ferns) on the southern slope of Mt. Mongón, a *lomas* mountain near Casma on the central northern coast of Peru (Figure \@ref(fig:study-area-mongon)).
...
During a field study to Mt. Mongón, all vascular plants living in 100 randomly sampled 4x4 m^2^ plots in the austral winter of 2011 [@muenchow_predictive_2013].
The sampling coincided with a strong La Niña event that year, as illustrated by monitoring of the [NOASS Climate Prediction Center](http://origin.cpc.ncep.noaa.gov/products/analysis_monitoring/ensostuff/ONI_v5.php).
...
Ordinations\index{ordination} are dimension-reducing techniques that allow the extraction of the main gradients from a (noisy) dataset, in this case floristic gradients as a function of environmental predictors including altitude, slope, catchment area\index{catchment area} and NDVI\index{NDVI}.
For this we will use a random forest model [@breiman_random_2001], resulting in maps of floristic composition across the study area.
Building on Section \@ref(svm) we will tune hyperparameters with the help of spatial cross-validation.
...
`mtry` should range between 1 and the number of predictors (`r ncol(task$data()) - 1`) <!-- (4)-->, `sample.fraction` should range between 0.2 and 0.9 and `min.node.size` should range between 1 and 10 [@probst_hyperparameters_2018].
...
```{r rf-pred, echo=FALSE, eval=TRUE, fig.cap="Predictive mapping of the floristic gradient clearly revealing distinct vegetation belts.", out.width="60%", fig.scap="Predictive mapping of the floristic gradient."}
# hs = terra::shade(terra::terrain(dem, v = "slope", unit = "radians"),
#                   terra::terrain(dem, v = "aspect", unit = "radians"),
#                   10, 200) |>
