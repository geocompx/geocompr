...
- You must have run the code in Chapter \@ref(intro) (as below)
...
# install.packages(pkgs_c3) # uncomment to install
lapply(X = pkgs_c3, FUN = library, character.only = TRUE)
if(!exists("world")) {
...
world = world %>% 
        mutate(set_units(st_area(.), value = km^2)) %>%
