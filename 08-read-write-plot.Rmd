...
Geographic metadata are a cornerstone of geographic information management. They are used to describe datasets, data structures and services to ensure data can be used properly. They are a key driver to make data findable accessible, inteoperable and reusable, and then contribute to the implementation of FAIR principles. Geographic metadata are defined by the ISO/OGC standards in particular the ISO 19115 standard and underlying schemas. These standards are widely used within spatial data infrastructures, handled through metadata catalogues. 

Through R, the management of geographic metadata can be done with the **geometa** package that allows writing, reading and validating geographic metadata according to the ISO/OGC standard schemas. It already supports various geographic metadata information international standards such as the ISO 19110 (Feature catalogue), ISO 19115-1 and 19115-2 (geographic metadata for vector and gridded/imagery datasets), ISO 19119 (geographic metadata for service), and ISO 19136 (Geographic Markup Language) providing methods to read, validate and write geographic metadata from R using the ISO/TS 19139 (XML) technical specification.

The creation of a geographic metadata is based on an object-oriented approach. A base metadata object is created and its fields can be completed with dedicated methods:
...
The package comes with [examples](https://github.com/eblondel/geometa/tree/master/inst/extdata/examples). Given the complexity and exhaustiveness of the ISO 19115 and associated standards, and the advanced knowledge required to use them, complementary packages such as [geoflow](https://github.com/r-geoflow/geoflow) have been built to ease and automate the management of metadata based on a simplified pivot metadata model that can be represented in a tabular form tightly coupled to data flows.

In the field of standard geographic information management, barrier between data and metadata is narrow. Indeed, the GML standard covers both data and metadata. The **geometa** package allows exporting GML (ISO 19136) objects from geometry objects modeled with **sf**. Such native GML support provided by geometa is currently unique in the R development community and provides a flexible framework to create objects from the broad GML data model where other libraries (such as GDAL) limit their support to the scope of datasets. Such capacity fosters the interoperability with geographic metadata (e.g. inclusion in metadata of detailed geographic extents, rather than simple bounding boxes; temporal extents) but also services that extend the GML standard (e.g. OGC Web Coverage Service).

**geometa** is a well-established and active project, with development plans to increase the coverage of existing standards (e.g. GML) and to adapt with the evolution of geographic information standards (e.g. ISO 19115-3).
