...
- `fill`: shaded areas for (mutli)polygons
- `borders`: border outlines for (mutli)polygons
- `polygons`: both, shaded areas and border outlines for (mutli)polygons
- `lines`: lines for (mutli)linestrings
- `symbols`: symbols for (mutli)points, (mutli)linestrings, and (mutli)polygons
- `raster`: colored cells of raster data
- `text`: text information for (mutli)points, (mutli)linestrings, and (mutli)polygons
...
The final map illustrated in Figure \@ref(fig:tmlayers) is created by adding a layer representing high points (stored in the object `nz_height`) onto the previously created `map_nz2` object with `tm_dots()` (see `?tm_dots` and `?tm_symbols` for details on **tmap**'s point plotting functions).
The resulting map, which has four layers, is illustrated in the right-hand panel of Figure \@ref(fig:tmlayers):

<!--toDo: jn-->
<!-- update when/if tm_dots is added -->
```{r 08-mapping-7}
# map_nz3 = map_nz2 +
...
```{r break-styles, message=FALSE, fig.cap="Illustration of different interval scales' methods set using the style argument in tmap.", , fig.scap="Illustration of different binning methods using tmap.", echo=FALSE}
...
```{r concat, message=FALSE, fig.cap="Illustration of continous and categorical scales in tmap.", echo=FALSE}
library(tmap)
library(spData)
library(spDataLarge)
m_cont1 = tm_shape(nz) +
...
Sequential palettes can be single (`greens` go from light to dark blue, for example) or multi-color/hue (`yl_gn_bu` is gradient from light yellow to blue via green, for example), as demonstrated in the code chunk below --- output not shown, run the code yourself to see the results!
...
We may decidedon its position using two arguments, where the first one can be `"left"`, `"center"`, or `"right"`, and the second one can be  `"bottom"`, `"center"`, or `"top"`.
...
Map elements include among others the objects to be mapped, the title, the scale bar, and margins, while the color settings covered in the previous section relate to the palette and break-points used to affect how the map looks.
Both may result in subtle changes that can have an equally large impact on the impression left by your maps.

Additional map elements such as north arrows\index{tmap (package)!north arrows}, scale bars\index{tmap (package)!scale bars}, and map titles have their own functions: `tm_compass()`, `tm_scale_bar()`, and `tm_title()` (Figure \@ref(fig:na-sb)).^[Another additional map elements include `tm_logo()` and `tm_credits()`.]
