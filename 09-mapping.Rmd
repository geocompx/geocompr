...
Scales control how the values are represented on the map and in the legend, and depends on the selected visual variable. 
...
By default, the used scale is set automatically given by the data type (factor, numeric, and integer).
<!-- At the same time, we are able to customize scales with a family of functions, including the `tm_scale()`,  -->
