...
The first group is used for categorical data without any particular order, for example, state names or land cover categories.
The colors should be easily distinguishable and, if possible, intuitive as in the case of blue rivers (and pastures green).
...
The second group, sequential palettes, are used for mapping quantitative data.
These palettes mostly follow a color gradient either for a single color (e.g. gradient from light to dark blue) or multi-hue color combination (e.g. a gradient from light yellow to dark green).
Light colors usually represent lower and dark colors higher values

<!-- GO ON FROM HERE -->
The third group, diverging palettes represent the departure of values from a reference point.
...
It assures that each unique value will be presented by its own category, and therefore color.
