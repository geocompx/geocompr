...
The interactive 'web maps' demonstrated in the previous sections can go far.
...
In the code above the **user interface** is created by `fluidPage()` which contains input 'widgets' --- a `sliderInput()` in this case but many other `*Input()` functions are available --- and outputs, a `leafletOutput()` in this case.
...
Armed with this basic example and knowing where to find help (see `?shiny`), the best way to learn how to create interactive web mapping applications with R is to stop reading and start programming!

```{r shiny, echo=FALSE, fig.cap="Minimal example of a web mapping application created with **shiny**."}
knitr::include_app("https://bookdown.org/robinlovelace/coffeeapp/")
```

To that end we have provided a more involved example that we recommend you modify.
**shiny** used in this way alongside makes prototyping flexible mapping applications faster and more accessible than ever before (deploying **shiny** apps is a separate topic beyond the scope of this chapter).
