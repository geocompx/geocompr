...
But it there are many other options for creating static maps.
The most mature option is to use `plot()` methods, which was in section \@ref(basic-map) and \@ref(basic-map-raster) for `sf` and `raster` objects respectively.
What we didn't mention in those sections was that plot methods for raster and vector objects can be used in combination, as illustrated in the subsequent code chunk which generates Figure \@ref(fig:nz-plot).
`plot()` has many options, documented in `?plot` and the vignette [`sf5`](https://cran.r-project.org/web/packages/sf/vignettes/sf5.html).
...
The syntax is similar to that used by tmap, with an initial `ggplot()` call followed by one or more layers.
While new **tmap** layers are added with `+ tm_*()`, **ggplot2** layers are added with `+ geom_*()`, where `*` should be replaced by a layer type in each case.

A useful feature of **ggplot2** is that it plots graticules by defult, illustrating how geographic shapes relate to the curvature of the Earth with lines of latitude and longitude.
