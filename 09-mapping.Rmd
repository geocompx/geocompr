...
The code below creates and launches --- with the command `shinyApp()` --- a minimal web mapping app to allow users to explore the global distribution of country-level life expenctancy data (see Figure \@ref(fig:lifeApp)):
...
The preceding code chunk creates the **user interface** via `fluidPage()` which contains input 'widgets' --- a `sliderInput()` in this case but many other `*Input()` functions are available --- and outputs, a `leafletOutput()` in this case.
