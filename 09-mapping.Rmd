...
Categorical legend could be not the most suitable when presenting a large number of objects (e.g. points or polygons) or continous fields (e.g. continous rasters).
In this situations, `cont` or `order` can be used. 
The former is appropariate for variables with evenly distributed values, while the latter works well when the distribution of values is skewed (for example, only a few raster cells have very large values).
<!-- cat -->
