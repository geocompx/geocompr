...
While this could be a complex decision to made, we can give some advice.
...
Their colors need to be easy to distinguish and they should, if possible, reflect the most common association.
Therefore, for most cases you should not experiment with red rivers and green deserts.
You also need to know the limitation of our own perception - while it is usually easy to distinguish several colors from each other, this task become more and more difficult with a growing number of categories.
...
It is usually presented by changes in color brightness either for a single (e.g. the `Blues` palette, light blue to dark blue) or multi-hue combination (e.g. the `YlGn` palette, light yellow to dark green).
