...
```{r}
main_w = norm_dim(nz_region)[["w"]]
main_h = norm_dim(nz_region)[["h"]]
```
...
```{r}
# using "snpc" unit to ensure width to height ratio stays constant
# independently of ROOT viewport dimensions
library(grid)
...
```{r}
ins_vp = viewport(x = unit(1, "npc") - unit(0.5, "cm"), y = unit(0.5, "cm"),
                  width = unit(ins_w, "snpc") * 0.5, height = unit(ins_h, "snpc") * 0.5,
                  just = c("right", "bottom"))
```
