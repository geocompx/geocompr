...
The default color scheme can be changed using `palette` argument.
...
While this could be a complex decision to made, we can give some advices.
...
Their colors need to be easy to distinguish and they should, if possible, relect the most common association.
Therefore, for most cases you should not experiment with red rivers and green deserts.
You also need to know the limitation of our own perception - while it is usually easy to destingush several colors from each other, this task become more and more difficult with a growing number of categories.
<!-- expand and test how tmap behavies with a large number of classes -->
<!-- sequential -->
Sequential palettes are used for the maps of quantitative data to show the order of values.
It is usually presented by changes in color brigtness either for a single (e.g. the `Blues` palette, light blue to dark blue) or multi-hue combination (e.g. the `YlGn` palette, light yellow to dark green).
Usually, lower values are presented by light colors and darker colors indicate larger values.
<!-- diverging -->
Diverging palettes are used to represent a departure of values from a reference mid-point.
For example, it could be a zero temperature in Celsius, median of a household income, probability of 50% for some event or a sex ratio of 1.
...
```{r break-styles, fig.cap="Illustration of different binning methods set using the syle argument in tmap.", echo=FALSE}
