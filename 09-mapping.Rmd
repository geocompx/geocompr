...
map_pkgs_df = dplyr::select(pkg_df, package, title)
...
The `HDI` values should be represented by one of the three groups - "High human development" (`HDI` above 0.7), "Medium human developement" (`HDI` between 0.55 and 0.7) and "Low human developement" (`HDI` below 0.55).
Additionally, try to improve the map aestetics, for example by changing the legend title, class labels or color palette.
```{r, eval=FALSE, echo=FALSE}
# >0.8 - Very high human development
# >0.7 - High human development
# >0.55 - Medium human developement
# <0.55 - Low human developement
sm1 = tm_shape(africa) + tm_polygons(col = "HDI", 
                               title = "Human Development Index",
                               breaks = c(0, 0.55, 0.7, 0.8),
                               labels = c("Low human developement",
...
im = tm_shape(utah) +
  ...
  tm_layout(title = "Utah", title.position = c("right", "top"))

library(grid)
print(sm3, vp = grid::viewport(0.5, 0.5, width = 0.95, height = 0.95))
print(im, vp = grid::viewport(0.25, 0.4, width = 0.35, height = 0.35))
```
<!-- animated map -->
<!-- @robin -->
<!-- interactive map -->
