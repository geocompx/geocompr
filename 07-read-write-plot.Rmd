...
knitr::kable(datapackages, caption = "Selected R packages for spatial data retrieval.", booktabs = TRUE) %>%
...
In an effort to standardise web APIs for accessing spatial data, the Open Geospatial Consortium (OGC) has created a number of specifications for web services (collectively known as [OWS](http://www.opengeospatial.org/standards/owc)).
...
Server software products such as PostGIS have adopted these standards, meaning that any data provided by such server-side technology can be provided using the same query system which, like other web APIs, use a 'base URL' and a 'query string' proceeding a `?` to request data.
An important request that any web service should be able to respond to is `getCapabilities`.

The following example shows how we can get the capabilities of a WFS which serves open geographic data from the Netherlands:

```{r, eval=FALSE}
u = "http://geodata.nationaalgeoregister.nl/bag/wfs?"
g = "GetCapabilities"
res = httr::GET(url = u, query = list(request = g))
...
In addition to sending requests with the **httr** package, the [**rwfs**](https://github.com/rOpenGov/rwfs) github package has been developed for working with WFS.
The package's documentation, and a short tutorial on WFS in [QGIS](https://publicwiki.deltares.nl/display/OET/Add+filtered+WFS+to+QGIS) are recommended resources;
other web services work in a similar way.
...
knitr::kable(file_formats, caption = "Selected spatial file formats.", booktabs = TRUE) %>% 
...
knitr::kable(head(sf_drivers, n = 5), caption = "Sample of available drivers for reading/writing vector data (it could vary between different GDAL versions).", booktabs = TRUE)
```
...
knitr::kable(dT, caption = "Data types supported by the raster package.", booktabs = TRUE)
```
