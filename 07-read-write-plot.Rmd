...
One of the most fundamental is `getCapabilities`, demonstrated with the **httr** package to show how API queries can be constructed and dispatched, in this case to discover the capabilities of a service providing run by the Fisheries and Agriculture Organization of the United Nations (FAO):
...
q = list(request = "GetFeatures")
httr::GET(url = base_url, query = q)
```
...
The code chunk below demonstrates how the **ows4R** package can be used to get all `FAO_AREAS` and a specific area using the `cql_filter` argument (results not shown):

```{r, eval=FALSE}
library(ows4R)
wfs = WFSClient$new("http://www.fao.org/figis/geoserver/wfs",
                      serviceVersion = "1.0.0", logger = "INFO")
areas_all = wfs$getFeatures("area:FAO_AREAS")
