...
Another way to access OWS is via the **leaflet** package, which provides the function `addWMSTiles()` for accessing WMTS data and visualising in an interactive map (see \@ref(adv-map) for more on web mapping).
...
library(ows4R)
csw = WFSClient$new(base_url, serviceVersion = "1.0.0", logger = "INFO")
csw$getCapabilities()
csw$getClassName()
records <- csw$getFeatures()
ows4R::OWSCapabilities$new(url = res$url, service = "wfs")
```
