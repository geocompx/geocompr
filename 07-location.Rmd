...
- This chapter requires the following packages (**ggmap** and **classInt** must also be installed):
...
People are fundamental to location analysis, in particular where they are likely spend their time and other resources.
However, ecological concepts and models can be useful for store location analysis.
Animals have needs can best meet their needs in certain 'optimal' locations, based on variables that change over space [@muenchow_review_2017]<!--and chapter xx-->.
...
- Where do target groups live and which areas to they frequent?
...
- What is the market share of a company in a specific are?

This chapter demonstrates how geocomputation can answer such questions based on a hypothetical case study with based on real data.
This case study is described in the next section.

## Case study: bike shops in Germany {#case-study}

Imagine you are starting a chain of bike shops in Germany.
A survey suggests that single young males (aged 20 to 40) are most likely to buy your products: this is the *target audience*.
You are in the lucky position to have sufficient capital to open a number of shops.
But where should they be placed?
Location analysts would happily charge high rates to answer such questions, but it is now possible for the answer to be generated for free using open data and open source software, in a reproducible manner, given sufficient geocomputational skill.
...
- Create geographic objects from the census data by converting the census data into raster format (section \@ref(create-census-rasters)).
...
Further, `mutate_all` is used to covert values -1 and -9 (meaning unknown) to `NA`.
...
Now the data has been preprocessed, it can be converted into a raster stack.
This is done by using the **tidyverse** function `map()` to iterate over each attribute variable (columns from 3 to 6 in `input_tidy`) in combination with the **raster** function `rasterFromXYZ()`.
The result is a list of raster objects containing data on each of the selected census variables, illustrated in Figure \@ref(fig:census-stack).

```{r}
input_ras = map(3:6, ~ rasterFromXYZ(input_tidy[c(1:2, .)]))
```
...
A cell value of 10,000 inhabitants was chosen for 'class 6' because these cells contain more than 8000 people.
...
```{r}
reclass = map2(input_ras, rcl, reclassify, right = NA) %>%
  set_names(names(input_tidy[3:6])) %>% 
  stack()
crs(reclass) = st_crs(3035)$proj4string
```

<!-- maybe a sentence or two of explanations could be important here -->
```{r, echo=FALSE, eval=FALSE}
reclass = stack(input_ras)
...
`st_transform()` is used before `rasterize()` to ensure the CRS of the inputs match.
...
This is acheived with the complimentary functions `addLayer()` and `dropLayer()`:
...
With clean data the final step, calculating of a final score by summing up all raster layers, can be accomplished in a single line.
