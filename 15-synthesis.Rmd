...
Which to use?
It depends.
The former only processes the geometry data contained in `nz` so is faster. latter two options performed attribute operations, which may be useful for subsequent steps.
...
This is a technical book and it makes sense for the next steps to also be of a technical nature.
But there are many non-technical issues to consider, now you know what is possible with geographic data in R.


The section is contains geo* rather than geocomputation because, as we say in chapter \@ref(intro), there are many definitions that capture the essence of we have attempted to teach in this book: geographic data science, geographic information systems, geoinformatics, different words for roughly the same thing.
Each captures ways of doing things with geographic data.
The practical aspect of geocompation emphasizes *what you do* with the methods, rather than the umbrella name of the term in its own right.
Who cares, what the work is called?
Hence the section heading 'Geo*', which indicates that this section, and much of the book, could equally be based on a different branch of the 'computing-for-geography' tree.
saw in section \@ref(what-is-geocomputation), geocomputation originated as a field that has three main ingredients:
...
We believe the broader non-technical aims make geospatial rewarding:
what is the point of building a new geographic method (tool) if its only purpose is to increase sales of perfume?
<!-- None. -->

Building on the three key components of geocomputation mentioned above and described in early work, we added another: reproducibility.
