...
The aim is to synthesize the contents, with reference to recurring themes/concepts, and to inspire future directions of application and development.
...
The final section (\@ref(benefit)) returns to the wider issues raised in Chapter \@ref(intro) and considers how geographic methods can have a positive impact on the world.
...
176 packages are mentioned in the Spatial [Task View](https://cran.r-project.org/web/views/) alone (as of summer 2018);
...
As indicated in the previous sections, the book has covered only a small fraction of the R's geographic ecosystem, and there is much more to discover.
...
<!-- This section is based on the premise that the key ingredients that make-up 'geocomputation', and maximise its ability to have a positive impact, depend on a wider . -->
Of course, how to define and measure 'positive' subjective:
it is a personal and philosophical beyond the scope of this book.
Regardless of your worldview, however, consideration the impacts of geocomputational work is a useful exercise:
...
These considerations lead to the conclusion that geocompuation is part of a wider 'open source approach', engagement with which can lead to tangible benefits for the people and organisations doing geocomputation and the wider community.
...
The open source approach to working with geographic data has advantages that transcend the technicalities of how the software works, encouraging learning, collaboration and an efficient division of labour.
...
A quick glance at the `r-spatial/sf` GitHub repository which hosts the code underlying the **sf** package, for example, shows that it has 42 'contributors' (people who have commited code improving the package) and dozens more people contributing by raising issues on its '[issue tracker](https://github.com/r-spatial/sf/issues)'.
...
Considering that **sf** is only one (relatively small) component in the wider R-spatial community provides a sense of the scale of the intellectual operation underway to make geocomputation with R possible and moving forward.
<!-- This is what we mean by the open source approach: collaboration between people with shared aims. -->

It is fun and instructive watch the incessant developement activity happen in public fora such as GitHub but it is even more rewarding to become an active participant.
...
Whether it's raising a constructive issue alerting developers to problems in their package; making the done by you and the organisations you work open; or simply helping out other people by passing on the knowledge you've learned, getting involved can be a rewarding experience.
