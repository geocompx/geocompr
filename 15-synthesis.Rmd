...
As of early 2019, there are nearly 200 packages (183) mentioned in the Spatial [Task View](https://cran.r-project.org/web/views/);
...
If you are interested in this topic, you can check for updates on the package's [source code](https://github.com/r-spatial/stars) and the broader [SpatiolTemporal Task View](https://cran.r-project.org/web/views/SpatioTemporal.html).
...
Section \@ref(what-is-geocomputation) presented other terms that mean roughly the same thing as geocomputation, including geographic information systems (GIS) and geographic data science (GDS).
...
Reproducibility supports *creativity*, encouraging the focus of methods to shift away from the basics (which are readily available through shared code, many many people from 'reinventing the wheel') and towards applications.
...
Considering that **sf** is only one component in the wider ecosystem of R's geographic offerings shows the scale of the intellectual operation underway to make geocomputation with R not only possible, but highly effective and continuously evolving.
