...
A characteristic of R is that there are often multiple ways to acheive the same result, and geocomputation with R is no exception.
Chapter \@ref(intro) mentions 20+ influential spatial packages that have been developed over the years.
If this seems like a lot, remember that there are hundreds of spatial packages and countless functions.
In the spatial task view alone xx packages are mentioned; more spatial packages are developed each year.
