...
It would be possible to study *Geocomputation with: Python*, *C++*, *JavaScript*, or *Scala* in equal depth.
...
that could supplement/replace the **raster** package used in this book; Python tools for working with geographic data are described in @garrard_geoprocessing_2016 and online tutorials such as those available at [automating-gis-processes.github.io](https://automating-gis-processes.github.io/CSC18).
Dozens of geospatial libraries have been developed in C++.
GDAL and GEOS are the most prominent of these but there are many others that may be worth exploring such as **[Orfeo Toolbox](https://github.com/orfeotoolbox/OTB)** for processing remote sensing (raster) data.
[**Turf.js**](https://github.com/Turfjs/turf) is a powerful package for geographic vector data analysis that demonstrates the potential of geocomputation with JavaScript (it also has implementations in other languages.
Scala is a growing Java-based language that supports raster and vector data in the GeoTrellis library, documentation on which can be found at [geotrellis.io](https://geotrellis.io/).
