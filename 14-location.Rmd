...
input = dplyr::select(census_de, x = x_mp_1km, y = y_mp_1km, pop = Einwohner,
                      ...
                      hh_size = HHGroesse_D)
# set -1 and -9 to NA
input_tidy = mutate(input, across(.fns =  ~ifelse(.x %in% c(-1, -9), NA, .x)))
```
...
cap = paste("Categories for each variable in census data from",
            ...
            "14.1 for their spatial distribution).")
knitr::kable(tab,
...
```{r 14-location-11, warning=FALSE}
...
```{r 14-location-12, warning=FALSE}
summary(pop_agg)
...
```{r 14-location-25, message=FALSE}
