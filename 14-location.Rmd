...
- The **z22** package provides convenient access to German census data and can be installed from GitHub with `pak::pkg_install("jslth/z22")`.
...
The **z22** package [@z22] provides convenient access to this data.
...
The official census data provides cell center coordinates in `x_mp` and `y_mp` columns ("mp" = "Mittelpunkt", German for midpoint).

The next step joins these data frames and filters out cells with missing values (coded as `-1`).
...
```{r 14-location-17, warning=FALSE, message=FALSE}
