...
Vectorization is a process of convering rasters into vectors^[This term should not be confused with a code vectorization.].
...
```{r raster-vectorization, echo=FALSE, fig.cap=".", warning=FALSE}
library(tmap)
library(grid)
cols = c("sand" = "rosybrown", "clay" = "brown", "silt" = "sandybrown")

p1 = tm_shape(grain) + 
  tm_raster(legend.show = FALSE, palette = cols[c(1, 3, 2)]) +
  tm_layout(outer.margins = rep(0.01, 4), 
            inner.margins = rep(0, 4))

p2 = tm_shape(grain_point) + 
  tm_bubbles(col = "layer", legend.col.show = FALSE, palette = cols[c(1, 3, 2)]) +
  tm_layout(outer.margins = rep(0.01, 4), 
            inner.margins = rep(0.05, 4))

p3 = tm_shape(grain_poly) + 
  tm_polygons("VALUE", legend.show = FALSE, palette = cols, lwd = 3) +
  tm_layout(outer.margins = rep(0.01, 4), 
            inner.margins = rep(0, 4))

p4 = tm_shape(grain_poly2) + 
  tm_polygons("VALUE", legend.show = FALSE, palette = cols, lwd = 3) +
  tm_layout(outer.margins = rep(0.01, 4), 
            inner.margins = rep(0, 4))
