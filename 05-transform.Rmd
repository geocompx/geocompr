...
- Image (co-)registration is the process of aligning one image with another (in terms of CRS and resolution). Registration becomes necessary for images from the same scene but shot from different sensors or from different angles or at different points in time.

In this section we will mainly deal with the co-registration (alignment) of images including a matching resolution,  and origin.
A matching projection is of course also required but has been covered already in section \@ref(reprojection-raster-geometries).
...
Vectorization is the process of converting continuous raster data into discrete vector data, i.e., points, lines or polygons.^[This term should not be confused with code vectorization.]
