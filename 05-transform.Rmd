...
The followin commands create a text string representing the Oblique Lambert azimuthal equal-area projection, and reproject the raster into this CRS, using the `bilinear` method:
...
) %>% knitr::kable(caption = "Number of rows, columns cell count and mean values of the original `con_raster` and projected `con_raseter_ea` datasets")
```
