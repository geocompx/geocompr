...
This chapter goes a step further, by show how R allows modification of the *geometry* underlying spatial datasets.

Transforming vector geometries is covered in section \@ref(geo-vec).
This includes geometric unions --- joining together shapes, the geometric side of attribute and spatial data aggregation --- and a range of operations that modify the geographic component of each feature in spatial datasets including simplifying, buffering, clipping and even shifting/scaling/rotating geometries using 'affine transformations'.
Advanced tranformations of vector geometries include type transformations (e.g. from few multipolygons to many polygons) and 'rasterization', which sets-up the next section.

Section \@ref(geo-ras) covers geometric transformations that change the position and size of pixels underlying raster data.
It teaches how to align raster datasets from diverse sources to enable the map algebra methods demonstrated in section \@ref(map-algebra) can be applied effectively using functions provided by the **raster** package (see section \@ref(raster-alignment)).
Section \@ref(geo-ras) also demonstrates how to do raster aggregation, in section \@ref(ras-agg).
...
We can verify that the CRS has changed using `st_crs()` as follows (note the important components of the output `EPSG: 27700` and `+units=m`):

```{r}
st_crs(london_proj)
```

Geometric operations on this projected version of the `london` dataset make sense.
...
par_old = par()
par(mfrow = c(1, 2))
plot(multilinestring_sf$geom, col = "black")
plot(linestring_sf2$geom, col = c("red", "green", "blue"))
par(par_old)
```
