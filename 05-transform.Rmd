...
) %>% knitr::kable(caption = "Key attributes original and projected (WGS84) raster datasets.", digits = 4)
```
...
) %>% knitr::kable(caption = "Key attributes original raster with continuous values and projected (Equal Area) raster datasets.", digits = 4)
```
...
```{r seine-simp, echo=FALSE, fig.cap="Comparison of the original data of the Seine, Marne and Yonne rivers and its simplified version.", warning=FALSE}
par_old = par()
...
```{r us-simp, echo=FALSE, fig.cap="Comparison of the original data of the contiguous United States and two simplified versions using functions from sf and rmapshaper packages.", warning=FALSE}
par_old = par()
...
```{r centr, warning=FALSE, echo=FALSE, fig.cap="Comparison between the outputs of centroid (black points) and 'point on surface' (red points) functions on New Zeleand's regions (left) and the Seine, Marne and Yonne rivers (right)."}
par_old = par()
par(mfrow = c(1, 2), mar = c(0, 0, 1, 0))
plot(nz$geometry)
...
```{r buffs, echo=FALSE, fig.cap="Buffers around the Seine of 5km (left) and 50km (right). Note the colors, which reflect the fact that one buffer is created per geometry feature.", fig.show='hold', out.width="50%"}
...
```{r single-cast, echo = FALSE, fig.cap="Examples of linestring and polygon objects created by casting a multipoint object into a new geometry type.", warning=FALSE}
par_old = par()
par(mfrow = c(1, 3), mar = c(0, 0, 1, 0))
...
```{r raster-vectorization1, echo=FALSE, fig.cap="Raster and point representation of the elevations raster dataset.", warning=FALSE}
