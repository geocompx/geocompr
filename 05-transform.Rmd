...
As an example, we here change the spatial resolution of `elev` from 0.5 to 1 decimal degree, that means, we aggregate by a factor of 2 (Fig. \@ref(fig:aggregate-example)).
...
Note that changing the resolution frequently also changes the origin as here is also the case with the `elev_agg` object.
