...
As an example, we here change the spatial resolution of `elev` from 0.5 to 2 decimal degree, that means, we aggregate by a factor of 4 (Fig. \@ref(fig:aggregate-example)).
...
elev_agg = aggregate(elev, fact = 2, fun = sum)
par(mfrow = c(1, 2))
plot(elev)
plot(elev_agg)
```
...
However, we have to specify a method how to fill the new cells, that means how we distribute the value of the input cell into the nine new cells.
...
However, this is to be expected, disaggregating **cannot** interpolate values at a finer resolution, it simply uses an interpolation algorithm.
