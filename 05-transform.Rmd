...
On the other hand, aligning several images is possible in R and this section shows how to do so.
...
elev_agg = aggregate(elev, fact = 5, fun = mean)
par(mfrow = c(1, 2))
plot(elev)
plot(elev_agg)
```
...
```{r}
elev_disagg = disaggregate(elev_agg, fact = 5, method = "bilinear")
identical(elev, elev_agg)
```
