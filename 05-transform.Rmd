...
Many properties of the new object differs from the previous one, which include the number of columns and rows (and therefore number of cells), resolution (transformed from meters into degrees), and extent, as illustrated in Table \@ref(tab:catraster).
...
) %>% knitr::kable(caption = "Key attributes original `cat_raster` and projected `cat_raster_wgs84` raster datasets.")
```
...
Rasters reprojection changes values and spatial properties, such as the number of cells, resolution, and extent.
...
) %>% knitr::kable(caption = "Key attributes original `con_raster` and projected `con_raster_ea` raster datasets.")
```
...
```{r, eval=FALSE}
mean(values(con_raster))
mean(values(con_raster_ea), na.rm = TRUE)
```
...
<!-- new zeleand example -->
```{r, echo=FALSE, eval=FALSE}
plot(rmapshaper::ms_simplify(world, keep = 0.5, keep_shapes = TRUE))
plot(rmapshaper::ms_simplify(world, keep = 0.05, keep_shapes = TRUE))
plot(rmapshaper::ms_simplify(world, keep = 0.005, keep_shapes = TRUE))
plot(rmapshaper::ms_simplify(world, keep = 0.0005, keep_shapes = TRUE))
plot(rmapshaper::ms_simplify(world, keep = 0.00005, keep_shapes = TRUE))
```
