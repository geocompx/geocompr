...
```{block2 type='rmdnote'}
The **dplyr** function `pull()` extracts a single variable as a vector.
```
...
```{block2 type='rmdnote'}
In most cases a data frame with a geometry list column is most usefully represented as an `sf` object rather than a `data.frame` because the list colum can only be used for creating maps and spatial operations (such) if R 'knows' it is a spatial object, defined by a spatial package such as **sf**.
It is possible to convert a data frame with a geometry list column, such as `left_join4`, into an `sf` object, as follows: `st_as_sf(left_join4)`. 
```
