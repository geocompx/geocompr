...
## Intro attribute data raster
<!-- Not sure if we should start with an intro here or if we should add it to the general intro in the beginning which so far only talks about sf -->
In Chapter \@ref{spatial-class} we learned that a raster is basically a rectangular grid with equally-spaced cells, and that one cell can contain only one value. 
This is in stark contrast to vector data where one feature can have multiple attributes. 
However, raster stacks and bricks can consist of multiple raster layers with each of them representing one attribute such as temperature, elevation and rainfall (see Chapter \@ref{spatial-class}).
In the coming subsections we will learn how to create, summarize and reclassify a raster, and how to extract cell values from one layer and multiple layers at once (subsetting).

## Creating and summarizing raster objects
The easiest way to create a raster is to read a raster file from disk or from a server with the help of the `raster::raster()` command.
...
Just printing the raster object to the console will yield the minimum and maximum value.
...
- by cell
- by coordinates
- by matrix notation i, j
- reclass



what about:
- raster aggregation (-> in fact a spatial operation, corresponds to polygon dissolving)
- raster merging (-> merging of two rasters is also a spatial operation)
- raster algebra (adding, substracting, squaring, etc.)
- crop, overlay, reclassify
- focal functions
