...
For example, you can also subset spatial data based on the spatial extent of another spatial object using the exact same functions you know from base R (`[`]) or from the **tidyverse** (`filter()`).
That is the skills you learn here are cross-transferable which is also why this chapter lays the foundation for the next chapter (Chapter \@ref(spatial-data-operations)).
