...
When using both packages in a single session, the function in the most recently attached package will be used, 'masking' the incumbent function.
...
To avoid this error message, we recommend attaching **dplyr** (or the metapackage **tidyverse** of which it is part) before running `library(raster)` which attaches the **raster** package.
To prevent ambiguity, we use the long-form function name, prefixed by the package name and two colons (usually omitted from R scripts for concise code): `dplyr::select()`.
<!-- todo: -->
<!-- Should we stop doing this after this shift to terra? (RL 2021-10) -->
```
...
Key to workflows using **dplyr** functions is the ['pipe'](http://r4ds.had.co.nz/pipes.html) operator `%>%`, which takes its name from the Unix pipe `|` [@grolemund_r_2016].
...
res = knitr::knit_child('_03-ex.Rmd', quiet = TRUE, 
