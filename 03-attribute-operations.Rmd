...
This chapter describes how to manipulate geographic objects based on attributes such as the names of bus stops and elevations of pixels in a raster dataset.
...
the `[` operator in base R, for example, works equally for subsetting objects based on their attribute and spatial objects, and you can also attributes using spatial joins.
...
Geographic vector datasets are well supported in R thanks to the `sf` class, which extends base R's foundational `data.frame` class.
Like data frames, `sf` objects have one column per attribute variable (such as 'name') and one row per *feature* (e.g., per bus station).
