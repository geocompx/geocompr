...
All **dplyr** functions including `select()` always return a data frame-like object. 
...
knitr::kable(data_frame(Symbol = operators, Name = operators_exp), caption = "Table of comparison operators that result in boolean (TRUE/FALSE) outputs.", booktabs = TRUE)
```
...
world %>% 
  ...
  top_n(n = 3, wt = pop) %>%
  st_set_geometry(value = NULL) %>% 
  knitr::kable(caption = "The top 3 most populous continents, and the number of countries in each.", booktabs = TRUE)
```
...
The result is an `sf` object identical to the orignal `world` object but with two new variables (with column indices 11 and 12) on coffee production.
