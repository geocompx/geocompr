...
Since the underlying mathematics is a special case of matrix algebra^[A position of an indiviudal raster cell corresponds to a position in space. 
Map algebra makes sure that the position stays the same when the cell value changes during some processing.
This one-to-one locational correspondence is also true in matrix algebra in the case of additions and substractions, but not in the case of multiplications, divisions, etc. 
By contrast, map algebra aslo retains the one-to-one locational correspondence for the latter operations.] (see above), we present them in this chapter (non-spatial attribute operations).
