...
As outlined in Chapter \@ref(spatial-class), **sf** provides support for simple features in R, with 'sf versions' of generic functions such as `plot()` and `summary()` for spatial vector data.
(Execute `methods("plot")`, for example, to see them.)

The mature `data.frame` class (and extensions to it such as `tibble` and `tbl`, used in the tidyverse) is the basis for data analysis in R.
**sf**'s ability to treat spatial data as  data frame Extending this system to work with spatial data has many advantages, allowing the power of R's data analysis capabilities, developed by expert programmers, statisticians and the wider community, to be transferred to geographic attribute data.
