...
Then, raster processing uses in fact map algebra, a special case of matrix algebra, which is why it is so much faster compared to vector processing.
...
Since the underlying mathematics is a special case of matrix algebra^[A position of an individual raster cell corresponds to a position in space. 
Map algebra makes sure that the position stays the same when the cell value changes during some processing.
This one-to-one locational correspondence is also true in matrix algebra in the case of additions and subtractions, but not in the case of multiplications, divisions, etc. 
By contrast, map algebra also retains the one-to-one locational correspondence for the latter operations.] (see above), we present them in this chapter (non-spatial attribute operations).
...
The raster object stores the corresponding look-up table or "Raster Attribute Table" (RAT) as a data frame in a new slot named `attributes` which you see when you print a 'ratified' raster to the console.
Use `levels()` to just retrieve the attribute table.
See the help page of the `ratify()` command for more information.


```{r cont_cate_rasters, echo = FALSE, fig.cap = "Raster with numberic values (left) and a raster with categorical values"}
...
Global operations are also useful for the computation of distance and weight rasters.
...
- raster aggregation/disaggregation/resample (changing spatial resolution)
 e1 <- extent(-10, 10, -20, 20)
> e2 <- extent(0, 20, -40, 5)
> union(e1, e2)
- raster merging (-> merging of two rasters is also a spatial operation), mosaicking
- raster::buffer, crop, mask
distancefromPoints
gridDistance
compareRaster, erase, extend
- crop/intersect
...
14. Use `data(dem, package = "RQGIS")`, and reclassify the elevation in three classes: low, middle and high.
Secondly, compute the NDVI (`data(ndvi, package = "RQGIS")`) and the mean elevation for each altitudinal class.
15. Apply a line detection filter to `data(dem, package = "RQGIS")`.
16. Calculate the NDVI of a Landsat image. 
Use the Landsat image provided by the **spDataLarge** package (`system.file("raster/landsat.tif", package="spDataLarge")`).
17. This [post](https://stackoverflow.com/questions/35555709/global-raster-of-geographic-distances) shows how to use `raster::distance()`.
