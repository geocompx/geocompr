...
They are different in that zonal filters can take on any shape. 
That means the unifying element is a class instead of a predefined filter window of size 3-by-3 pixels.
Our grain size raster is a good example.
...
A true raster operation is of course to assign the mean values to the raster cells in question.
This implies we would have to replace our grain sizes by the mean elevation
