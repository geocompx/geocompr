...
library(tmap)
qtm(left_join1, "unemploy", fill.breaks = c(6, 6.5, 7))
```
...
### Raster subsetting
You certainly have noticed, that we have already used raster subsetting in the last code example of the previous subsection. 
The `cellFromRowCol()` retrieved the cell IDs for the specified row-column combinations.
These cell IDs in turn were used to extract the corresponding cell values with the help of the `[` operator.
The subsetting operator also accepts as input:
...
We demonstrate the first two subsetting options in the three commands below, all of which return the value of the top left pixel in the raster object `r` (results not shown):
...
For example, `stack(r, r_2)[1]` returns a matrix with one row and two columns - one for each layer.
...
We present spatial subsetting in this chapter because the returned output is non-spatial, that is just a vector representing the cell values to be found within the specified spatial constraint.
