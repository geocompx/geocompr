...
```{r, warning=FALSE}
cofee_data$name_long[grepl("Congo,", cofee_data$name_long)] = 
  str_subset(world$name_long, "Dem*.+Congo")
world_coffee_match = inner_join(world, cofee_data)
...
This is demonstrated below, which starts with the `cofee_data` object and adds variables from the original `world` dataset.
...
```{r, warning=FALSE}
coffee_world = left_join(cofee_data, world)
class(coffee_world)
```
