...
In the following, we first explain how to create and subset a raster. 
Subsequently, we classify raster operations into four categories using the map algebra scheme [@Tomlin_1990_geographic], and present each of these classes individually by example.

The easiest way to create a raster object in R is to read-in a raster file from disk or from a server.
For instance, to load a rasterized version of the famous `meuse` dataset, you can use `raster(system.file("external/test.grd", package = "raster"))`.
To support numerous drivers, the **raster** package uses **rgdal** under the hood.
...
Additionally, we define a resolution of 1, which here corresponds to one degree since the default `proj4string` of a raster object is WGS84.
Finally, we set the values with the `vals` argument.
As we have seen in Chapter \@ref{spatial-class}, this corresponds to a rowwise cell filling  starting at the upper left corner.
Consequently, the upper first row contains the values 1, 2 and 3, the second row 4, 5 and 6 and the last row 7, 8 and 9.

```{r}
library(raster)
r = raster(nrow = 3, ncol = 3, res = 1, 
...
r_2 = raster(nrow = 3, ncol = 3, res = 1, 
             xmn = -1.5, xmx = 1.5, ymn = -1.5, ymx = 1.5,
             vals = factor(rep(grain_size, each = 3), levels = grain_size))
```
...
### Map algebra
Map algebra (or cartographic modeling) divides raster operations into four subclasses [@Tomlin_1990_geographic], with each of them either working on one or several grids simultaneously:
...
In the following paragraphs, we will explain each of these operations by example.
...
To make a spatial prediction, all we have to do, is to apply the estimated coefficients to the predictor rasters, and summing up the resulting output rasters (Chapter ??; see also @Muenchow_2013_predictive).
<!-- add reference to chapter ecological modeling -->

While local functions operate on one cell, though possibly from multiple layers, focal operations take into account a target cell and its neigbors.
The neigborhood under considertation is typically of size 3 x 3 cells (that is one target cell and the eight surrounding neighbors) but can take on any other (not necessarily rectangular) shape as defined by the user.
The output of a focal operation can be the mean (or any other function) of all cells within the moving window.
Alternatively, one can also provide a weight matrix, say the eight neigbouring cells should only have a weight of 0.5, while the center has a weight of 2.

Image filtering is probably the most common use case for focal operations.
...
```
 ...
 filtering (sog. Filterfunktionen zu nennen, die in der Bildverarbeitung eine große Rolle spielen (vgl. Kap. 10.6.4),) in image processing (smoothing), slope, aspect calculation
see the help for the creation of a cellular automat to run the Game of Life

INKREMENTELLE Operatoren gehen entlang von vorgegebenen ein-, zwei- oder dreidimensionalen Geoobjekten vor (z.B. entlang einer Kette von Zellen oder über ein Gelände hinweg). Die Berechnungen von Abflussrichtungen und Abflusswegen ausgehend vom Höhenwert eines Pixels sind wichtige Beispiele.
