...
`aggregate()` is a generic function which means that it behaves differently depending on its inputs. **sf** provides the method `aggregate.sf()` which is activated automatically when a `by` argument is provided, rather than using `~` to refer to the grouping variable:
