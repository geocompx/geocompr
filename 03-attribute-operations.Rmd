...
The package provides spatial versions of generic functions such as `plot()` and `summary()` for exploring vector data.
(Execute `methods("plot")`, for example, to see them.)
A key feature of **sf** is that it can represent any vector dataset as a `data.frame` (or extensions to it such as `tibble` and `tbl`, used in the tidyverse), the basis of data analysis workflows in R.
This has many advantages.
In essence **sf** enables the full power of R's data analysis capabilities to be applied seemlessly to geographic data.
