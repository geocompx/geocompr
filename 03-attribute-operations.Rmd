...
Finally, our weighting matrix consists only of 1s.
This means each cell has the same weight on the output.
Naturally, you can change this by specifying different weights.
...
The `terrain()` function lets you compute of few of these terrain characteristics but has not implemented all popular methods, for example, the the Zevenbergen and Thorne method to compute the slope is missing.
...
Incremental operators fall also into the class of neighborhood operations.
