...
In the case of raster data we will learn how to create, summarize and reclassify a raster, and how to extract cell values from one layer and multiple layers at once (subsetting).
Finally, we will also have a glimpse on raster algebra which is basically matrix algebra (see above), which is why we predominantly consider it a non-spatial operation.

## Summarizing and subsetting vector attributes
...
## Creating and summarizing raster objects
...
This is why the `summary()` and `plot()` function will return numbers instead of classes.
...
## Raster subsetting and manipulation
...
To modify cell values, we overwrite existing values with the help of subsetting operations.
For instance, let us replace the upper left cell with a value of 12:
...
Another common raster operation is to convert values into classes, for example one might to divide a digital elevation model into low (class 1), middle (class 2) and high elevations (class ).
Using the `reclassify()` command, we need to construct a reclassification matrix, where the first column corresponds to the lower, the second column to the upper end of the class and the third column represents the new value for this range.
...
## Raster algebra (adding, substracting, squaring, etc.)

just raster algebra, and therefore not really a spatial calculation
- focal functions
