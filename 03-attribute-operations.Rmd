...
As outlined in Chapter \@ref(spatial-class), **sf** provides support for simple features in R.
The package provides spatial versions of generic functions such as `plot()` and `summary()` for exploring vector data, which can be seen using `methods()`:
...
Many of these function, including `rbind()` (for binding rows of data together) and `$<-` (for creating new columns) were developed for data frames.
This is a key feature of **sf**: it can represent any vector dataset as a `data.frame` (or extensions to it such as `tibble` and `tbl`, used in the tidyverse), allowing data analysis workflows to apply for spatial data (as illustrated in the functions above).
