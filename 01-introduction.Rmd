...
<!-- to do things sounds rather vague without context-->
...
<!--
"R for GIScience" or "GIScience with R" would have not implied GUI-based geoprocessing. The same goes for "Geo-data science with R"; maybe we should point out more prominently that Leeds was the birth place of geocomputation, and therefore the natural choice for the title :)
-->
...
<!--nowadays part-->
Nowadays, we use satellites, GPS, and multitudes of other sensors, and especially computational power to gain insights.
The latter is of special importance with regard to the ever increasing amount of (geo-)data.
For instance, an autonomous car is believed to create one terrabyte of data on a daily base.
We need software and related tools to extract the most interesting patterns of such data.
In this book we treat R as a 'tool for the trade' when it comes to analyzing such data.
However, data management and storing is better left to other software such as dedicated (geo-)database software.
R is an object-oriented, [functional programming language](http://adv-r.had.co.nz/Functional-programming.html) [@wickham_advanced_2014].
Furthermore, R was specifically designed as an interactive interface to other software [@chambers_extending_2016]. 
This also includes many 'bridges' to a treasure trove of GIS software, libraries and functions.
<!--
...
Another really impressive example showing R's flexbility with regard to geographic software development is its support for generating interactive maps thanks to **leaflet** [@R-leaflet].
