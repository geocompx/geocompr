...
This book is about harnessing the power of modern computers to 'do' exciting things with geographic data.
...
This also includes advanced geostatistical and spatial modeling as well as geocomputation and mapping.^[[RStudio](https://www.rstudio.com/) greatly fascilitates the usage of this rich programming suite as well as the development of software and code in R.
It also is one of the best available integrated developer environments (IDE).]
...
Though we will almost exclusively use R for geocomputation throughout this book, there are of course other sofware packages available for geocomputation.
Here, we will shortly introduce the most commonly used: C++, Java and Python with a focus on the latter.

A natural choice for geocomputation would be C++ since most major GIS packages (e.g., GEOS, GDAL, QGIS, GRASS, SAGA, and even ArcGIS) often rely in great parts on it.
...
Java is another important (and versatile) language in GIScience.
For example, the open-source vector GIS [OpenJump](http://openjump.org/) is written in the Java programming language.
Java's object-oriented syntax is similar to C++, but much simpler.
...
There are many open source add-on libraries available for Java, including [GeoTools](http://docs.geotools.org/). 
Overall, Java has an excellent performance on large-scale systems making it a suitable candidate as the basis for a desktop GIS.
However, it is probably less suitable for statistical modeling and visualization compared to Python or R.
@wood_java_2002 provides a good introduction if you want to learn more about Java and geocomputation.
<!-- ask Sven Kralisch for input here -->

Finally, there is Python for geoprocessing.
We will dwell a little bit more on Python since many people believe that R and Python are battling for supremacy in the field of data science.
This is accompanied by a partly offensive and often rather subjective discussion on what to learn or what to use.
...
So if you like Python better or you think it better suits your needs (for example because you also are also interested in web and GUI development), go for it.
...
In addition, Python also offers an excellent support for dealing with spatial data (see packages **osgeo**, **Shapely**, **NumPy**, **osgeo**, **PyGeoProcessing**). 
...
Alongside other packages that have a shared style and emphasis on 'tidy data' (including e.g., **ggplot2**), these were placed in the **tidyverse** 'metapackage' in late [2016](https://cran.r-project.org/src/contrib/Archive/tidyverse/).
...
The following issue of R News (Volume 1/3) also put spatial packages in the spotlight, with an introduction to **splancs** and a commentary on future prospects regardubg spatial statistics [@bivand_more_2001].
...
A package not mentioned in these overviews was **maptools** written by Nicholas Lewin-Koh [@bivand_maptools:_2017]. 
Initially, it just contained a wrapper around [shapelib](http://shapelib.maptools.org/), and permitted the reading of ESRI Shapefiles into geometry nested lists. 
The corresponding and nowadays obsolete S3 class called "Map" stored this list alongside an attribute dataframe. 
...
Two years later, @hornik_approaches_2003 published an extended review of spatial packages. 
Around this time the development of R's spatial capabilities increasinlgy supported interfaces to external libraries, especially to GDAL and PROJ.4.
...
In the following, **sp** classes rapdily became the go-to standard for spatial data in R, resulting in a proliferation of packages that depended on it from around 20 in 2008 and over 100 in 2013 [@bivand_applied_2013].
...
Therefore, Colin Rundel started to develop a package that interfaces GEOS, an open-source geometry library, during a Google Summer of Coding project.
In the following, the **rgeos** package [first released in 2010; @R-rgeos] brought geometry calculations to R by allowing functions and operators from the GEOS library to manipulate **sp** objects.
...
However, its main purpose was the easy access of several APIs to automatically download map tiles (among others, GoogleMaps, OpenStreetmap) and subsequent plotting of these as a basemap. 
...
To make spatial data work with the ggplot system, one needed to fortify spatial objects.
Basically, this means, you need to combine the coordinates and attribute slots of a spatial class object into one dataframe.
...
**tmap** provides legends, scale bars, grid lines, and north arrows optimized for mapping, and also works in harmony with the **leaflet** package (released in 2015) for an easy display of interactive maps [@R-leaflet]. 
...
However, it is noteworthy that among all the recent developments described above, the support for simples features [**sf**; @R-sf] has been without doubt the most important evolution in R's spatial ecosystem.
...
2. Provide three reasons for using a scriptable language such as R for geocomputation instead of using an established GIS programs such as QGIS.

3. Name two advantages and two disadvantages of using the older **sp** package compared to the new **sf** package.
