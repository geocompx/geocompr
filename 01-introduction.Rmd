...
(\@ref(gis)).
-->
It is thus ideal for quickly creating 'geo-tools', without needing to master lower level languages (compared to R) such as C, FORTRAN and Java (section \@ref(other-software-for-geocomputation)). 
...
What is more, advanced users might even extend R with the power of other languages (e.g., C++ through **Rcpp** or Python through **reticulate**; see also section \@ref(sec:other-software-for-geocomputation)).
...
## Other software for geocomputation
...
A natural choice for geocomputation would be C++ since most major GIS packages (QGIS, GRASS, SAGA, and even ArcGIS) are built on it.
C++ is blazzingly fast, and allows high-performance implementations.
...
Java is another important language in GIScience. 
<!-- ask Sven Kralisch for input here -->

Finally, there is Python for geoprocessing.
We will dwell a little bit more on Python since many people believe that the two are battling for supremacy in the field of data science.
...
However, when the teaching moves on to statistical geoprocessing and spatial predictive modeling we guide them to R where they can take advantage of the concepts already learned through Python.
