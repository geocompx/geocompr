...
```{r interactive, fig.cap="World at night imagery from NASA overlaid by the authors' home locations to illustrate interactive mapping with R."}
library(leaflet)
popup = c("Robin", "Jakub")
leaflet() %>%
...
```
 ...
 book teaches Geocomputation with reference to real-world examples and reproducible code rather than abstract concepts.
...
- Geography, of which Geocomputation is a part, is the study of where things are located on the Earth. Primarily we are interested in the part of Geography that deals with *geographic data* and *maps*: information with a location and the visualization of such data.
