...
Learning how to use the wealth of geospatial tools available from the R command line can be exciting and liberating.
However it is even more liberating to create new ones.
...
The major advantage of Python is that it is a *general-purpose* programming language allowing, among others, desktop software, game, front-end web development and data science whereas R is solely used for the latter [^The R package **shiny** allows some frontend web development functionality.].
This also explains Python's larger user base compared with R's.
Python is often the least common denominator between different (programming) fields which is probably also the reason why it has become the 'glue' that holds many GIS programs together.
