...
In this chapter we will introduce predictive mapping by means of statistical learning [@james_introduction_2013] while using spatial cross-validation for a bias-reduced assessemnt of the model performance - something which is probably only easily doable with R at the moment.
...
This is becauase a GLM is probably familiar to most reader, and therefore we can focus on why and how to do a spatial cross-validation instead of explaining in detail the model building.^[Readers who are in need of refreshing their regression skills might have a look at @zuur_mixed_2009.]
Nevertheless, a generalized additive model or a machine learning approach would be more suitable for our dataset.
...
These can be roughly grouped into supervised and unsupervised techniques (clustering). 
Here, we will deal with supervised 
