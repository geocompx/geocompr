...
```{r lsl-map, echo=FALSE, fig.cap="Landslide initiation points (red) and points unaffected by landsliding (blue) in Southern Ecuador. Randomly selected test points are marked by a golden border. CRS: UTM zone 17S (EPSG: 32717)."}
library(tmap)
...
The basic idea is to split a dataset times into training and test sets whereby the training data is used to fit a model which then is applied to the test set.
Comparing the predicted values with the known response values from the test set (using a performance measure such as the AUROC in the binomial case) gives an bias-reduced assessement of the model's capability to generalize the learned relationship to independent data.
For example, to use a 10-repeated 5-fold cross-validation means to randomly split the data into five partitions (folds) with each fold using a 1/5 of the available data as test and 4/5 as training data (see upper row of Figure \@ref(fig:partitioning)). 
...
Remember that the first law of geography states that points close to each other tend to be, on average, more similar compared to points further away.
This means these points are not statistically independent.
...
To overcome this problem, we should make use of spatial partitioning which partitions the observations spatially using its coordinates as input for k-means clustering (second row of Figure \@ref(fig:partition); @brenning_spatial_2012).
The partitioning strategy is the great difference between classical and spatial cross-validation.
The package **sperrorest** implemented spatial cross-validation in R [@brenning_spatial_2012].
In the meantime, it was integrated into the **mlr** package which is the reason why we will present it in more detail next.

```{r partitioning, fig.cap="Spatial and random partitioning.", echo = FALSE}
