...
But remember this single interface provides access to the 100+ learners shown by `listLearners()`; it would be far more tedious to learn the interface for each learner!
Further advantages are simple parallelization of resampling techniques and the ability to tune of hyperparameters (see section \@ref(svm)).
...
This takes a some time (around 10 seconds on a modern laptop) it computes the AUROC for 500 models. 
Setting a seed ensures that when re-running the code the same spatial partitions will be used for reproducibility of results.
...
To put these results in perspective let's compare them with AUROC values from a 100-repeated 5-fold non-spatial cross-validation (Figure \@ref(fig:boxplot-cv); the code for the non-spatial cross-validation is not shown here but will be explored in the exercise section).
...
The **mlr** is now set-up to fit 250 models to determine optimal hyperparameters for each fold.
...
Model runtime can be reduced with parallelization, which can be done in a number of ways, depending on operating system (there are more multicore processing on Unix-based systems than on Windows).
...
We will use half of the available cores (set with the `cpus` parameter), a setting that allows the computation to use dozens of cores a high performance computing cluster (HPC).
...
Specifying the `resample()` parameters follows the exact same procedure as presented when using a GLM, the only difference being the use of hyperparameter tuning results with the `extract` argument.
This is important if we plan follow-up analyses on the hyperparameter tuning.
After the processing, it is good practice to explicitly stop the parallelization with `parallelStop()`.
Finally, we output object (`result`) to disk in case we would like to use it another R session.
...
Running 125,000 models took around 37 minutes on a server using 24 cores.
...
Resampling methods are an important part of the data scientist's toolbox [@james_introduction_2013]. 
...
The **mlr** package facilitates (spatial) resampling techniques in combination with the popular statistical learning techniques including linear regression, semi-parametric models such as generalized additive models and machine learning techniques such as random forests and SVMs, and boosted regression trees [@bischl_mlr:_2016;@schratz_performance_nodate].
...
- A tutorial on [Predictive modeling with caret  by Zev Ross](http://www.zevross.com/blog/2017/09/19/predictive-modeling-and-machine-learning-in-r-with-the-caret-package/).
