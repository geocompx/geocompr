...
This chapter assumes proficiency with spatial raster data, for example gained from chapters \@ref(spatial-class) to \@ref(transform).
A familiarity with linear regression and its generalized extensions and machine learning is highly recommended [for example from @zuur_mixed_2009;@james_introduction_2013].
...
Both can be categorized into supervised and unsupervised techniques and are central to many disciplines including economics, physics, medicine, biology, ecology and geography [@james_introduction_2013].
...
CV helps to detect over-fitting since models that predict the training data too closely, with noise, will tend to performance poorly on the test data.

Randomly splitting spatial data can lead to training points that are neighbors in space with test points.
Due to spatial autocorrelation, test and training datasets would not be independent in this scenario, with the consequence that CV fails to detect a possible over-fitting.
...
First, it is worth demonstrating how spatial data can be modeled using the conventional modeling interface in 'base R'.^[
