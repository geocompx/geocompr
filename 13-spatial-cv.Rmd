...
Here, we neglect spatial autocorrelation when making predictions since we assume that on average predictive accuracies of parametric model remain the same with or without spatial autocorrelation structures.
However, it is possible to include spatial autocorrelation structures.
It is beyond the scope of this book to show how you could do so but instead we give you some pointers where to look it up:


Kriging approaches
1. Universal kriging (but only available for linear regression)  <!-- check -->, see also for a GAM example and how to do it manually, basically fitting a model and then using the residuals to interpolate the spatial component

Incorporating spatial autocorrelation explicitly in the model structure
1. GLS
1. glmmPQL

random intercept approaches

1. random-effect modeling sometimes helps to account for spatial autocorrelation [@zuur_mixed_2009].
"In the mixed modeling approach an extra residual term is added to the model that adds a dependency structure on the response variable.
This is especially useful if we have different locations where we can assume that the individuals of each location are more similar to each other in comparison with individuals from other locations, e.g., bee hives, owl nests, vegetation transects, etc.
Here, random intercepts are assumed to be normal and independent distributed.
1. The mixed modeling approach can be extended by using a random intercept that is normal and spatially dependent.
Most likely, you will have to resort to Bayesian modeling approaches for this since frequentist software tools are rather limited in this respect especially for more complex models [@blangiardo_spatial_2015; @zuur_beginners_2017]. 
