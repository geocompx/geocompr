...
We use cross-validation to determine how good a model is able to predict indepedent samples, <!--i.e. if our model fits only good our observations (overfitting) or if it is generalizable to -->
Therefore, we need data for which we already know the truth, e.g. if a landslide occurred at a specific location or not.
To do this we split our dataset into a training dataset which is used for estimating the coefficients and a test dataset.
For the latter we already know the result and comparing this with the predicted result for this data, we can determine the misclassification rate (or any other kind of error measure) or thus our model's performance. 
<!-- add a few lines saying what is cv and why it is important! -->
