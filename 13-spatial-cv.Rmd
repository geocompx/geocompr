...
This way we introduce statistical supervised modeling in R. 
Additionally, this will contribute to get a better grasp on the **mlr** approach introduced later on, and provides the required skillset for doing spatial CV.
...
The `response` option gives back the predicted probabilities for each of observation in `lsl`, i.e. the probability that a landslide has occurred (see `? predict.glm`).
...
This function also expects the fitted model as input as well as a raster stack containing the same predictors we have used in the model as raster objects.

```{r}
pred = raster::predict(object = ta, model = fit, type = "response")
plot(pred, col = RColorBrewer::brewer.pal(name = "Reds", 6))
```

```{r, echo = FALSE}
asp = terrain(ta$elev, opt = "aspect")
hs = hillShade(ta$slope * pi / 180, asp, 40, 270)
hs = mask(hs, study_area)
plot(hs, col = gray(seq(0, 1, length.out = 100)), legend = FALSE)
# plot(pred, col = RColorBrewer::brewer.pal("YlOrRd", n =  9), add = TRUE, 
#      alpha = 0.6)
plot(mask(pred, hs), col = RColorBrewer::brewer.pal(name = "Reds", 6), add = TRUE, 
     alpha = 0.6)
```

Here, when making predictions we neglect spatial autocorrelation since we assume that on average predictive accuracies of parametric model remain the same with or without spatial autocorrelation structures.
...
1. The predictions of universal kriging are the predictions of a simple linear model plus the kriged model's residuals, i.e. interpolated residuals [@bivand_applied_2013]. 
...
1. Mixed-effect modeling approaches
...
Classes can be for example different bee hives, owl nests, vegetation transects or an altitudinal stratification.
