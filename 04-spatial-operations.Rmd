...
Plotting them shows that they are closely related but they do not touch.
This can be verfied in Figure \@ref(fig:cycle-hire), a static version of which is generated by the plotting code below, and with `st_intersects()` as show below:
...
```{r, warning=FALSE}
sum(st_touches(cycle_hire, cycle_hire_osm, sparse = FALSE))
```
...
```{r cycle-hire, fig.cap="The spatial distribution of cycle hire points in London based on official data (blue) and OpenStreetMap data (red)."}
library(leaflet)
leaflet() %>%
