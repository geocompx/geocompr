...
Section \@ref(attribute-subsetting) describes methods for attribute subsetting using base R and the **tidyverse**. The same methods can be used for spatial subsetting, once you understand how.
As in chapter \@ref(attr), we start with base methods before describing how to do it in the **tidyverse**.
...
This is a concise and consistent syntax that we illustrate with a hypothetical scenario: subsetting all countries in the world that lie within 20 degrees of the point where the equator (where latitude = 0 degrees) intersects the prime meridian (longitude = 0 degrees), as illustrated in Figure \@ref(fig:globe).
...
```{r buffeq, fig.cap="Subset of the `world` data selected based on their intersection with a circle 20 degrees in radius with a center point at 0 degrees longitude and 0 degrees latitude."}
