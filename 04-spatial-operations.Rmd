...
urb = urban_agglomerations %>% 
  filter(year == 2020) %>% 
  top_n(n = 4, wt = population_millions)
asia = world %>% 
...
The default of `st_join()` is to intersect the two layers.
Of course, you can use any other of the topological operations we have already encountered above such as `st_within()` or or `st_touches()` (please refer also to the help page of `st_join()`.
In the example above, we have added features of a point layer to a polygon layer.
Of course, we can use other vector classes as well.
For instance, polygons and lines or lines and points.
Please note that there might be multiple matches for a polygon. 
Had we chosen to select the four most populous cities in the world, two of them would belong to China (Shanghai and Beijing).
In such a case `st_join()` simply adds a new row, i.e., in our example we would end up with two polygons representing China.
In most cases this is not what you want.
<!-- finally, refer to left and inner -->
...
### Modifying geometry data - dissolving and clipping
