...
```{r nz-subset, echo=FALSE, warning=FALSE, fig.cap="Illustration of spatial subsetting with red triangles representing heigh points in New Zealand. The right-hand map contains only points in the Canterbury region (highlighted in grey). The points were subset with `nz_height[canterbury, ]`."}
par_old = par()
...
Before we progress to the next section it is worth seeing another way of doing spatial subsetting.
This relies on *topological operators*'s ability to return `logical` objects --- when the argument `sparse` is set to `FALSE`:
...
`sel` is a matrix with a row per feature in the target object and a column per feature in the source object (`nz_height` with 101 features and `canterbury` with ` feature respectively in this case).
If there is more than 1 feature in `y` the resulting selection `matrix` must be converted into a `vector` before it is used for subsetting:
