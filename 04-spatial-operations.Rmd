...
```{r areal-example, echo=FALSE, fig.cap="Illustration of congruent (left) and incongruent (right) areal units with respect to larger aggregating zones (translucent blue borders).", fig.asp=0.5, out.width="100%", fig.scap="Illustration of congruent and incongruent areal units."}
...
Just like focal operations, *zonal* operations aggregate values of multiple raster cells.
Several aspects distinguish zonal from focal operations:

- while the spatial filter in `focal()` is defined as a fixed matrix (to represent the moving window), 
the *zonal filters* (or 'zones'), used to aggregate cell values, are areas defined by the unique values of a *second raster*.
- consequently, the raster cells defining the zonal filter do not necessarily have to be neighbours.
...
- the result of a zonal operation is a matrix with summary statistics per zone, while focal operations return a raster.

This kind of operation is also known as *zonal statistics* in the GIS world. 

To find the mean elevation for each grain size class (respectively left and right panels of Figure \@ref(fig:cont-raster)), we use the `zonal()` function.
