...
The first way to reproduce the subsetting operation illustrated in Figure \@ref(fig:buffeq) simply involves expanding the operation over 2 lines:

```{r}
sel_relate = st_intersects(x = world, y = buff_equator, sparse = FALSE)
world_buff_equator2 = world[sel_relate,]
```



Furthermore it will demonstrate the foundations of a 'tidy' spatial data analysis workflow that you'll find familiar if you already use **dplyr** for data manipulation.

How can we be sure that the results obtained through the 4 subsetting operations demonstrated above?
We can test it as follows:

```{r}
identical(x = world_buff_equator, y = world_buff_equator2)
```
