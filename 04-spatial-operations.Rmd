...
### Spatial subsetting
Naturally, cell ID and index subsetting have a spatial counterpart.
To extract values from raster objects, we can also use coordinates and spatial objects.
To use coordinates, we have to 'translate' them into the corresponding cell ID(s) or by using the `extract()` command.
...
# the same as
extract(r, data.frame(x = -1.5, y = 1.5))
```
...
We can also use a raster object to subset another raster object (Figure \@ref(fig:raster-subset)).
...
# extract(r, extent(r_3))
```

Basically, this amounts to retrieving the values of the first raster (here: `r`) falling within the extent of a second raster (here: `r_3`).
To retrieve a spatial output, we can tell R to keep the matrix structure.
This will return the four output values as a raster object.
...
```{r raster-subset, echo = FALSE, fig.cap = "Subsetting raster values with the help of another raster."}
knitr::include_graphics("figures/03_raster_subset.png")
```

Frequently, however, we have two rasters with the same extent and resolution where one raster object serves as a mask.
In these cases `intersect()` and `crop()` are of little use.
Instead we can use the `[` again or the `mask()` and `overlay()` commands.

```{r, eval = FALSE}
r_4 = raster(nrow = 6, ncol = 6, res = 0.5, 
           xmn = -1.5, xmx = 1.5, ymn = -1.5, ymx = 1.5,
           vals = sample(c(FALSE, TRUE), 36, replace = TRUE))
r[r_4, drop = FALSE]
# using the mask command
mask(r, r_4, maskvalue = TRUE)
# using overlay
# first we replace FALSE by NA
r_4[r_4 == FALSE] = NA
# then we retrieve the maximum values
overlay(r, r_4, fun = "max")
```

In the code chunk above, we have created a mask object called `r_4` randomly using `FALSE` and `TRUE`.
Next we only want to keep those values of `r` which are `TRUE` in `r_4`, or expressed differently, we want to mask `r` with `r_4`.
These operations are in fact boolean local operations since we cellwise compare two rasters.
The next subsection explores these and related operation in more detail.
...
Computing a distance raster (zonal operation) while only considering a maximum distance (logical focal operation) is the equivalent to a vector buffer operation.
Reclassifying raster data (either local or zonal function depending on the input) is equivalent to dissolving vector data. 
Overlaying two rasters (local operation), where one contains `NULL` or `NA` values representing a mask, is similar to vector clipping.
Finally, we can intersect or union two rasters with the help of the boolean operations AND and OR.

```{r mapalg_vector, echo = FALSE}
d = 
  tibble("vector operation" = 
           c("buffer (section \\@ref(modifying-geometry-data))",
             "dissolve (section \\@ref(spatial-joining-and-aggregation))",
             "clipping (section \\@ref(modifying-geometry-data))",
             "intersection/union (section \\@ref(spatial-subsetting)))"),
         "raster operations" = 
           c("distance raster rectricted to a threshold",
             "reclassify",
             "binary raster zonal operation",
             "boolean operation OR/AND")
       )
knitr::kable(x = d, caption = "Map algebra vector equivalents.")
```
buffer distance raster within a specified threshold
dissolve reclassifying 
