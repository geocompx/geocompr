...
Like attribute subsetting, the command `x[y, ]` subsets features of a *target* `x` using the contents of a *source* object `y`.
...
Specifically, objects used for spatial subsetting in this way must have the class `sf` or `sfc`.
