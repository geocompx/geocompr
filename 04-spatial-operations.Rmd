...
### Spatial subsetting
In the previous chapter (\@ref(manipulating-raster-objects)) we have already learned how to subset raster datasets using cell IDs and matrix indexing.
...
These operations are in fact boolean local operations since we compare cellwise two rasters.
The next subsection explores these and related operations in more detail.

###  Map algebra: local, focal, zonal, global
Map algebras makes raster processing really fast.
This is because we only implicitly store coordinates with raster datasets.
...
In R, we would have to merge/mosaic the two overlapping rasters. 
Of course, this requires the two rasters to share the same resolution (and origin) <!--find out-->.
Subsequently, we could use boolean operations to find the overlapping area.
...
d = 
  tibble("vector operation" = 
           c("buffer (section \\@ref(modifying-geometry-data))",
             ...
             "intersection (section \\@ref(spatial-subsetting)))"),
         "raster operations" = 
           c("distance raster rectricted to a threshold",
             "reclassify or zonal statistics",
             "boolean operation",
             "boolean operation")
       )
