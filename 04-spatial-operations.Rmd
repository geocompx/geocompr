...
This question can also be answered using spatial predicates such as `st_intersects()` follows:

```{r 04-spatial-operations-9, eval=FALSE}
st_intersects(point_sf, polygon_sfc)
#> Sparse geometry binary predicate... where the predicate was `intersects'
#>  1: 1
#>  2: (empty)
#>  3: 1
```
...
the function returns a positive (`1`) for the first and third point, and a negative result (represented by an empty vector) for the second are outside the polygon's border.
...
A more restrictive question is: which points lie within the polygon?
The can be answered as follows:

```{r 04-spatial-operations-9-2, eval=FALSE}
st_within(point_sf, polygon_sfc)
#> Sparse geometry binary predicate... where the predicate was `within'
#> 1: (empty)
#> 2: (empty)
#> 3: 1
```
...
This only applies to the third point, which is inside the polygon, as illustrated below:
