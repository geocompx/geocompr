...
plot(urb["population_millions"], pch = 15, main = "Source object (points)", key.pos = NULL)
plot(asia$geom, border = "grey", add = TRUE)
plot(urb["population_millions"], pch = 15, main = "Result of spatial join", key.pos = NULL)
plot(asia$geom, add = TRUE)
...
```{r spatial-aggregation, echo=FALSE, warning=FALSE, fig.cap="Illustration of spatial aggregation. The right map represents the average value of points in the left map, aggregated to the regional level in New Zealand."}
par(mfrow = c(1, 2))
breaks = seq(from = 2700, to = 3300, by = 100)
plot(nz$geometry, main = "Point elevation")
plot(nz_height["elevation"], key.pos = NULL, add = TRUE, breaks = breaks)
plot(nz$geometry, main = "Average elevation")
plot(nz_avheight["elevation"], key.pos = NULL, breaks = breaks, add = TRUE)
par(mfrow = c(1, 1))
```
...
```{r us-regions, fig.cap="Spatial aggregation on contiguous polygons, illustrated by aggregating the population of US states into regions, with population represented by color. Note the operation automatically dissolves boundaries between states.", echo=FALSE, warning=FALSE, fig.asp=0.4}
# with base R ----
old_par = par()
...
# 
# qtm(us_states, c("total_pop_15", "region_pop"))
# 
# qtm(us_facet, "total_pop_15") +
#   tm_facets(by = "Level", nrow = 1)
```
