...
The simplest method is to use the topological operator `st_is_within_distance()`, as demonstrated below using a threshold distance of 20 m.
Note that this works with projected data and also with unprojected data.
In the latter case, however, one needs to make sure that the spherical geometry engine (s2) is switched on which it is by default in **sf** (see section \@ref(s2)).
