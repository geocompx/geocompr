...
```{r 14-eco-18, echo=FALSE, eval=TRUE}
library("tree")
tree_mo = tree(sc ~ dem, data = rp)
```
...
The first internal node at the top of the tree assigns all observations which are below `r gsub("<", "", as.character(as.data.frame(tree_mo$fram$splits)[1, "cutleft"]))` to the left and all other observations to the right branch.
The observations falling into the left branch have a mean NMDS\index{NMDS} score of `r round(tree_mo$frame$yval[2], 3)`.
...
```{r 14-eco-26, echo=FALSE}
tune = readRDS("extdata/14-tune.rds")
```

An `mtry` of `r tune$x$mtry`, a `sample.fraction` of `r tune$x$sample.fraction`, and a `min.node.size` of `r tune$x$min.node.size` represent the best hyperparameter\index{hyperparameter} combination.
A RMSE\index{RMSE} of `r round(tune$y[attr(tune$y, "names") == "rmse.test.rmse"], 2)` is relatively good when considering the range of the response variable which is `r round(diff(range(rp$sc)), 2)` (`diff(range(rp$sc))`).
...
#           gp = gpar(cex = 0.8))
knitr::include_graphics("https://user-images.githubusercontent.com/1825120/72242126-8a739b00-35e0-11ea-8088-ca6c2b113bf4.png")
```
