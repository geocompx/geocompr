...
- introduce desicion trees
- shortly explain bagging
- random forests, improves bagging by decorrelating trees
- explain hyperparameters `mtry` and `num.trees`

<!--
Recall that bagging is simply a special case of a random forest with m = p. Therefore, the randomForest() function can random be used to perform both random forests and bagging. 
...
The preparation for the modeling using the **mlr** package includes the construction of a separate coordinate dataframe and the construction of the response-predictor matrix.
...
ps = makeParamSet(
  makeIntegerParam("mtry", lower = 1, upper = ncol(d) - 1),
...
# ranger(sc ~ ., data = d, mtry = tune$x$mtry, num.trees = tune$x$num.trees)
```
...
However, there are also approaches that help to quantify variable importance when using machine learning. 
- We have used just the first NMDS axis.
It would be interesting to also model the second axis and to subsequently finding an innovative way of visualizing jointly the modeled scores of the two axis in one prediction map.
