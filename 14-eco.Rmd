...
^[Admittedly, it is a bit unsatisfying, that the only way of knowing that `sagawetnessindex` computes the desired terrain attributes, is to be familiar with SAGA and/or google for "GIS catchment slope computation".]
...
1. instead of the AUROC which can only be used for categorical response variables, we will use the root mean squared error as performance measure.
...
In the previous section we retrieved bias-reduces performance estimates

## Ecological context

## Exercises
1. Compute catchment area and catchment slope using **RSAGA**.
1. Use profile and tangential curvature as additional predictors (hint: `grass7:r.slope.aspect`), and find out if this improves the prediction.
