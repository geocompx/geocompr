...
```{r, echo=FALSE, fig.cap="Study mask (polgyon), location of the sampling sites (black points) and DEM in the background."}
...
```{r tree, fig.cap="Simple example of a decision tree with three internal nodes and four terminal nodes."}
library("tree")
tree_mo = tree(sc ~ dem, data = rp)
plot(tree_mo)
text(tree_mo, pretty = 0)
```
<!-- rounding is wrong in the online figure output, ok, save the figure as png and include it separately -->

The resulting tree consists of three internal nodes and four terminal nodes (Figure \@ref(fig:tree)).
The first internal node at the top of the tree assigns all observations which are below `r gsub("<", "", as.character(as.data.frame(tree_mo$fram$splits)[1, "cutleft"]))` m to the left and all other observations to the right branch.
The observations falling into the left branch have a mean NMDS score of `r round(tree_mo$frame$yval[2], 3)`.
Overall, we can interpret the tree as follows: the higher the elevation, the higher the NMDS score becomes.


- shortly explain bagging
- random forests, improves bagging by decorrelating trees
- explain hyperparameters `mtry` and `num.trees`
...
The preparation for the modeling using the **mlr** package includes the construction of a response-predictor matrix containing only variables which should be used in the modeling and the construction of a separate coordinate dataframe and t.
...
To tune the hyperparameters we will use a five-fold spatial partitioning with only one repetition (`makeResampleDesc()`). 
...
Specifically `mtry` should range between 1 and the number of predictors (`r ncol(rp) - 1`), and `num.trees` should range between 10 and 10,000.
...
tune = tuneParams(learner = lrn_rf, 
                  ...
                  measures = mlr::rmse)
#>...
#> [Tune-x] 49: mtry=2; num.trees=5221
#> [Tune-y] 49: rmse.test.rmse=0.5896348; time: 0.0 min
#> [Tune-x] 50: mtry=4; num.trees=5813
#> [Tune-y] 50: rmse.test.rmse=0.5092251; time: 0.0 min
#> [Tune] Result: mtry=4; num.trees=696 : rmse.test.rmse=0.4994471
```
...
An `mtry` of `r tune$x$mtry` and a `num.trees` of `r tune$x$num.trees` represent the best hyperparameter combination.
A RMSE of `r round(tune$y[attr(tune$y, "names") == "rmse.test.rmse"], 2)` is relatively good when considering the range of the response variable which is `r round(diff(range(rp$sc)), 2)` (`diff(range(rp$sc))`).
...
lrn_rf = makeLearner(cl = "regr.ranger",
                     ...
                     mtry = tune$x$mtry, num.trees = tune$x$num.trees)
# train model
model_rf = train(lrn_rf, task)
# to retrieve the ranger output, run:
# mlr::getLearnerModel(model_rf)
# which corresponds to:
# ranger(sc ~ ., data = rp, mtry = tune$x$mtry, num.trees = tune$x$num.trees)
```
...
```{r rf-pred, echo=FALSE, fig.cap="Predictive mapping of the floristic gradient clearly revealing distinct vegetation belts.", fig.width = 10, fig.height = 10}
library("latticeExtra")
library("grid")
...
The **mlr** package provided the building blocks to spatially tune the hyperparameters `mtry` and `num.trees` (section \@ref(mlr-building-blocks).
...
- If we were interested in interpreting the model in an ecological meaningful way, we should use (semi-)parametric models [@muenchow_predictive_2013;@zuur_mixed_2009;@zuur_beginners_2017].
However, there are at least approaches that help to quantify variable importance when using machine learning learning algorithms such as random forests.
...
1. Retrieve the bias-reduced RMSE using spatial cross-validation including the estimation of optimal hyperparameter combinations in an inner tuning loop (see section \@ref(svm)).
