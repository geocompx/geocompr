...
```{r study-area-mongon, echo=FALSE, fig.cap="The Mt. Mongón study area, from Muenchow, Schratz, and Brenning (2017).", out.width="60%"}
...
```{r, echo=FALSE, message=FALSE, fig.cap="Study mask (polygon), location of the sampling sites (black points) and DEM in the background."}
...
```{r xy-nmds, fig.cap="Plotting the first NMDS axis against altitude.", fig.asp=1, out.width="60%"}
...
p1 = xyplot(scores(rotnmds)[, 2] ~ scores(rotnmds)[, 1], pch = 16, 
             ...
             # points
             par.settings = simpleTheme(col = c("grey", "black"), 
                                        pch = 16, cex = 0.9), 
             # also the legend point size should be somewhat smaller
             auto.key = list(x = 0.7, y = 0.9, text = c("2011", "2012"), 
                             between = 0.5, cex = 0.9),
             panel = function(x, y, ...) {
                           # Plot the points
...
```{r, eval=FALSE, echo=FALSE}
png("figures/14_tree.png", width = 6, height = 5, units = "cm", res = 300)
par(mar = rep(0.4, 4))
plot(tree_mo)
text(tree_mo, pretty = 0)
dev.off()
...
```{r tree, echo=FALSE, fig.cap="Simple example of a decision tree with three internal nodes and four terminal nodes."}
