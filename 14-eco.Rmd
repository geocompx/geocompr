...
This mean, the inner hyperparameter tuning level is no longer needed which makes perfectly sense since we are applying our model to new data (unvisited field observations) for which the true outcomes are unavailable, hence testing is impossible in any case. 
...
We will use a regression since the response variable is numeric.
...
An `mtry` of `r res$x$mtry` and a `num.trees` of `r res$x$num.trees` represent the best hyperparameter combination.
A RMSE of `r res$y[attr(res$y, "names") == "rmse.test.rmse"]` is relatively good when considering the range of the response variable which is `r diff(range(d$sc))` (`diff(range(d$sc))`).
The tuned hyperparameters can now be used for the prediction.
We simply have to modify our learner accordingly, and run the corresponding model.
