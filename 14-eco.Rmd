...
For the latter, one estimates the best hyperparameter combination on the complete dataset.
...
Hyperparameter combinations will be selected randomly but fall inside specific tuning limits (`makeParamSet()`), specifically `mtry` can range between 1 and the number of predictors (`r ncol(d) - 1`), and `num.trees` can range between 10 and 10,000.
...
Finally, `tuneParams()` runs the hyperparameter tuning, i.e. will find the optimal hyperparameter combination for the specified parameters.
The performance measure it the root mean squared error (RMSE).
...
A RMSE of `r tune$y[attr(tune$y, "names") == "rmse.test.rmse"]` is relatively good when considering the range of the response variable which is `r diff(range(d$sc))` (`diff(range(d$sc))`).
...
So far, `raster::predict()` does not support `ranger()` models, hence, we will have to do so ourselves.
First, we convert `ta` into a prediction dataframe which secondly serves as input for the `predict()` function.
Thirdly, we put the predicted values back into a raster (Figure \@ref(fig:rf-pred)).

```{r}
# convert raster stack into a dataframe
new_data = as.data.frame(as.matrix(ta))
...
The blue color tones represent the so-called *Tillandsia*-belt [@muenchow_soil_2013].
`Tillandsia` is a highly adapted genus often found in high quantities at the sandy and quite desertic foot of *lomas* mountains.
...
Water availabilitly naturally decreases above the temperature inversion, and the landscape becomes desertic again only harboring a few succulent species (succulent belt; red colors).
