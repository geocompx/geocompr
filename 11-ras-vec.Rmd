...
```{r pointext, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Locations of points used for raster extraction."}
library(tmap)
library(grid)
...
```{r lineext, echo=FALSE, message=FALSE, warning=FALSE}
library(tmap)
library(grid)
...
grid.text("B. Transect elevation", vp = viewport(layout.pos.row = 1, layout.pos.col = 2))
...
```{r polyext, echo=FALSE, message=FALSE, warning=FALSE}
library(tmap)
library(grid)
...
rast_poly_nlcd = tm_shape(nlcd) +
  tm_raster(col = landcover_names, palette = landcover_cols, style = "cat", title = "Land cover") + 
  tm_shape(zion) +
  tm_borders(lwd = 2) +
  tm_layout(legend.frame = TRUE, legend.position = c("right", "top"))
