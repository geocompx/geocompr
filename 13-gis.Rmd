...
Additionally, he would like to set out his journy a this home, and come back to it after
...
Even if one iteration can be done in a ms this still corresponds to (factorial(100 - 1) / (1000 * 60 * 60 * 24 * 365)) years
...
Before using GRASS, we need to add data to its spatial database. Luckily, the convenience functions `writeVECT` and `writeRAST` do this for us. Here, we only transfer our street and cycle point vector data.
```{r}
