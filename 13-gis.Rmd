...
The U.S. Army - Construction Engineering Research Laboratory (USA-CERL) created the core of the hybrid raster-vector Geographical Resources Analysis Support System (GRASS) \@ref(tab:gis_comp) [@neteler_open_2008] from 1982 to 1995. 
...
Suppose a traveling salesman would like to visit 99 customers while covering the shortest distance possible.
...
There is a single best solution to this problem, however, to find it, is even for modern computers impossible (**citation**).
In our case, this corresponds to factorial(100 - 1) / 2 (we do not differentiate between back and forth).
Even if one iteration can be done in a ms this still corresponds to `r (factorial(100 - 1) / (2 * 1000 * 60 * 60 * 24 * 365))` years. 
...
In our use case, we would like to find the shortest path between 25 randomly selected bicycle stations (instead of customers) on London's streets.
...
We download it with the help of the **osmdata** package.
...
Now that we have the data, we can go on and initiate a GRASS session (basically a geodatabase), i.e., we have to define a mapset, a location, a projection and the boundaries of our GRASS project [for more information see @neteler_open_2008].
You have to do the same if you wanted to use the GRASS GUI. 
This might be a reason why inexperienced GIS users at times find GRASS a bit intimidating in the beginning.
Fortunately, **link2GI** greatly fascilitates this tedious but important task.
...
^[Of course, you can also use GRASS6 but then you would also have to use the `spgrass6`-package.]
...
linkGRASS7(streets, link[grepl("7.2", link[, "version"]), ])
# linkGRASS7(streets_int, c("C:/OSGeo4W64", "grass-7.2.1", "osgeo4W"))
```

^[
Here's how you do it manually. We save our geodatabase to a temporary folder, and name the location `london` and the mapset `tsp`.

```{r, eval = FALSE}
library(rgrass7)
ind <- grepl("7.", link$version)
grass_path <- ifelse(link$installation_type[ind] == "osgeo4W",
                    file.path(link$instDir[ind], "apps/grass", link$version[ind]),
                    link$instDir)
initGRASS(gisBase = grass_path,
          gisDbase = tempdir(),
          location = "london", mapset = "tsp", override = TRUE)
```

]
Before using GRASS, we need to add data to its spatial database.
Luckily, the convenience function `writeVECT` does this for us.
Use `writeRast` in the case of raster data.
In our case we add the street and cycle hire point data, and name them also `streets` and `points`. 
```{r}
writeVECT(SDF = as(streets, "Spatial"), vname = "streets")
writeVECT(as(points, "Spatial"), vname = "points")
```
...
Our cycling station points not necessarily lie on a street.
To find the shortest route between them, we need to connect them to the streets.
`v.net`'s connect-operator does exactly this, it connects the points to to the nearest street segment (check visually if this is true!!). 
...
Finally, we can find the shortest route between all points using the `v.net.salesman`-algorithm.
