...
For this reason this chapter focusses on 'bridges' to the mature GIS products [QGIS](http://qgis.org/) (via the package **RQGIS**), [SAGA](http://www.saga-gis.org/) (**RSAGA**) and [GRASS](https://grass.osgeo.org/) (**rgrass7**) from within R. Obviously, we here focus on open-source software solutions, however, there is also a bridge to the commercial GIS leader [ArcGIS](https://www.arcgis.com) through the **RPyGeo** package. And the so-called [R-ArcGIS bridge](https://github.com/R-ArcGIS/r-bridge) allows to use R from within ArcGIS.
...
`run_qgis` prints these paths to the console after sucessfully runing the QGIS engine.
...
Though SAGA is a hybrid GIS, its main focus has been on raster processing, and here particularly on digital elevation models (soil properties, terrain attributes, climate parameters). Hence, SAGA is especially good at the fast processing of large (high-resolution) rasters data sets [@conrad_system_2015]. 
Therefore, we will introduce **RSAGA** with a raster and use case from @muenchow_geomorphic_2012. First of all, we need to tell **RSAGA** where to find SAGA on the computer.
Usually, `rsaga.env()` will detect SAGA automatically. However, we have 'hidden' SAGA in the OSGEO4W-installation (-> RSAGA feature request), which is why we have to indicate the corresponding path:
...
```{r}
data("landslides")
write.sgrd(data = dem, file = file.path(tempdir(), "dem"),
           header = dem$header, env = env)
```
 
The organization of SAGA is modular. Libraries contain so-called modules (or geoalgorithms). To find out which libraries are available, run (instead of presenting all available libraries we only show the last ten):

```{r}
tail(rsaga.get.libraries(path = env$modules), 10)
```

Subsequently, we can access the available modules of a specific library (here: 'ta_morphometry') as follows:

```{r}
rsaga.get.modules(libs = "ta_hydrology", env = env)
```

Similarly to **RQGIS** `get_usage()`, `rsaga.get.usage()` finds the function paramters of a specific geoalgorighm.
...
in other SAGA versions also called: 'Catchment Area (Top-Down)' or 'Flow Accumulation (Top-Down)'

 
Finally, you can run SAGA from within R using **RSAGA**'S geoprocessing workhorse function `rsaga.geoprocessor()`. The function expects a parameter-argument list.
...
rsaga.geoprocessor(lib = "ta_hydrology", module = "SAGA Wetness Index",
                   param = params, env = env)
```

RSAGA frequently provides user-friendly wrapper-functions with meaningful default values (see RSAGA documentation for examples). So the function call for calculating the 'SAGA Wetness Index' becomes as simple as:

```{r}
rsaga.wetness.index(in.dem = "dem", 
                    out.wetness.index = file.path(tempdir(), "twi"),
                    env = env)
```

You can find a much more extended version of the here presented example in the RSAGA vignette `vignette("RSAGA-landslides")` including statistical geocomputing. The term statistical geocomputation emphasizes the strength of combining R's data science power with the geoprocessing power of a GIS which is at the very heart of building a bridge from R to GIS.
...
**link2GI** fascilitates this tedious task. So instead of defining all the stuff yourself, you call:
...
So you should stick to **RSAGA** and **rgrass7** if you are also using less common GIS functionalities.
