...
This is also highlighted by its description of 'legacy GIS' on its [website](http://workshops.boundlessgeo.com/postgis-intro/introduction.html).
Similar to GRASS, PostgreSQGL provides a (partial) GUI called [pgAdmin](https://www.pgadmin.org/) to fascilitate the editing and administration of the database. 
...
<!--
maybe we should delete the half sentence in parentheses because neither was RStudio designed for spatial analysis. And PostGIS was certainly also intended to be accessible through other applications. Or maybe I am not sure what you are referring to. What do you mean by CLI, just the console? If so, how does it differ substantially from other CLIs? I have the impression you are mostly talking about RStudio, right? 

PostGIS also lacks a sophisticated CLI With the partial exception of [pgAdmin](https://www.pgadmin.org/) (which was not designed for handling spatial data), and many users interact with it through GUIs such as that provided by QGIS.
-->
...
- facilitates the automation of repetitive tasks. We strongly disklike the manual execution of iterations, and would recommend to always think about how to solve such tasks programmatically.
...
If you are missing an algorithm you need, the CLI gives you the freedom you to write your own!
...
- map production, in case you only want to create a beautiful map once. If you have to produce it over and over again, then maybe the CLI approch is better.
...
Similarly to `RQGIS::get_usage()`, `rsaga.get.usage()` prints the function paramters of a specific geoalgorithm, e.g., the `SAGA Wetness Index`, to the console.
...
To fasciliate the access to the SAGA interface, RSAGA frequently provides user-friendly wrapper-functions with meaningful default values (see RSAGA documentation for examples, e.g., `?rsaga.wetness.index`).
...
**link2GI** fascilitates this tedious task.
