...
Similar to SAGA, GRASS focussed on raster processing in the beginning while only later adding advanced vector functionality (since GRASS 6.0).
...
First of all, the GRASS database requires its own directory, and contains a location \@ref(fig:grass_loc).
...
In order to share spatial data with all users of a project, the database own can add spatial data to the PERMANENT mapset.
...
# Eventually, we will get rid off the if-else and of the link-dataframe given
# that this issue is resolved: https://github.com/gisma/link2GI/issues/2
if (Sys.info()["sysname"] == "Windows") {
  loc <- linkGRASS7(streets, link[grepl("7.2", link[, "version"]), ])
} else {
  loc <- linkGRASS7(streets)
}
...
^[
Here is how you do it manually. We save our geodatabase to a temporary folder, and name the location `london` and use the PERMANENT mapset.
...
initGRASS(gisBase = grass_path,
          ...
          home = tempdir(),
          gisDbase = tempdir(), location = basename(tempfile()), 
          mapset = "PERMANENT")
...
Before using GRASS geoalgorithms, we need to add data to our spatial database.
...
We save `v.nets`'s output in `streets_points_con`.
...
To visualize our result, we import the output layer into R, and visualize it with the help of the **mapview** package.
