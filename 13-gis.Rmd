...
The organization of SAGA is modular. Libraries contain so-called modules (or geoalgorithms).
To find out which libraries are available, run (instead of presenting all available libraries we only show the last ten):
...
To fasciliate the access to the SAGA interface, RSAGA frequently provides user-friendly wrapper-functions with meaningful default values (see RSAGA documentation for examples). So the function call for calculating the 'SAGA Wetness Index' becomes as simple as:
...
You can find a much more extended version of the here presented example in the RSAGA vignette `vignette("RSAGA-landslides")` including statistical geocomputing.
