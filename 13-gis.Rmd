...
```{r saga_twi, fig.cap = "SAGA wetness index of Mount Mongón, Peru."}
library(raster)
...
- unified access to several GIS, and therefore the provision of >1000 geoalgorithms.
...
- automatic data format conversions.
...
- convenience functions to support the access of the online help, R named arguments and automatic default value retrieval.
...
Use **RSAGA**'s numerous user-friendly functions when doing so.
...
In case you need topological correct data and/or geodatabase-management functionality, we recommend the usage of GRASS. 
In addition, if you would like to run simulations with the help of a geodatabase, use **rgrass7** directly since **RQGIS** always starts a new GRASS session for each call.
