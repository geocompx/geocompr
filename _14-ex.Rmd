...
library(osmdata)
library(spDataLarge)
# the raster package is also needed but will be referenced with :: syntax in due
# course
```
...
input = dplyr::select(census_de, x = x_mp_1km, y = y_mp_1km, pop = Einwohner,
                      ...
                      hh_size = HHGroesse_D)
input_tidy = mutate(input, across(.fns = ~ifelse(. %in% c(-1, -9), NA, .)))
...
input = dplyr::select(input, x = starts_with("x_mp_1"), y = starts_with("y_mp_1"),
                        inh = Einwohner)
# set -1 and -9 to NA
input = mutate(input, across(.fns = ~ifelse(. %in% c(-1, -9), NA, .)))
# convert table into a raster (x and y are cell midpoints)
inh_fine = terra::rast(input, type = "xyz", crs = "EPSG:3035")
# CHECK: contains the fine raster the number of inhabitants or only inhabitant classes
# (like its coarse counterpart?)

# Comparing the coarse with the fine raster
#******************************************

# aggregate to the resolution of the coarse raster
inh_fine = terra::aggregate(inh_fine, 
                            fact = terra::res(inh_coarse)[1] / terra::res(inh_fine)[1], 
                            fun = sum, na.rm = TRUE)
...
terra::global((abs(inh_fine - inh_coarse) > 1000), stat = "sum", na.rm = TRUE)
terra::global((abs(inh_fine - inh_coarse) > 5000), stat = "sum", na.rm = TRUE)
```
