...
E1. Load the population data at 100 m cell resolution using `z22_data("population", res = "100m", year = 2022)`.
Aggregate it to a cell resolution of 1 km using `terra::aggregate()` with `fun = sum`, and compare the result with the 1 km resolution data loaded directly with `z22_data("population", res = "1km", year = 2022)`.
...
# Load 1 km population data directly
...
# Load 100 m population data (this may take some time)
pop_100m = z22_data("population", res = "100m", year = 2022, as = "df")
pop_100m = pop_100m |>
  rename(pop = cat_0) |>
  mutate(pop = ifelse(pop < 0, NA, pop)) |>
  mutate(x = x - 50, y = y - 50)  # Correct z22 coordinate offset (50m for 100m resolution)
