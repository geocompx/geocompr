...
```{r stations, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Station nodes (red dots) used as intermediary points that convert straight desire lines with high rail usage (black) into three legs (red)."}
zone_cents = st_centroid(zones)
zone_cents_rail = zone_cents[legs,]
plot(desire_rail$geometry)
plot(legs$geometry, add = TRUE, col = "red")
