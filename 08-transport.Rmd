...
ABM is a powerful approach to transport research with great potential for integration with R's spatial classes [@thiele_marries_2014; @lovelace_spatial_2016], but is outside the scope of this chapter.
...
Often the geographic units of destinations are the geographic units that comprise the origins, but a different zoning system, such as '[Workplace Zones](https://data.gov.uk/dataset/workplace-zones-a-new-geography-for-workplace-statistics3)', may be appropriate to represent the increased density of trip destinations in central areas [@officefornationalstatistics_workplace_2014].
...
The resulting table provides a snapshot of Bristolian travel patterns.
...
```{r desire, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Desire lines representing trip patterns in the Bristol Travel to Work Area. The four black lines represent interzonal OD pairs in Table 8.1."}
# tmap figure
...
<!-- st_length works with sf 0.6.0 but not with 0.6.1 -->
```{r, message=FALSE}
...
<!-- not entirely sure what you mean here: are you talking about connecting a node outside of the network by finding the closest node of the graph network? -->
...
After researching Bristol's transport system at multiple levels we can now tie-together the various strands covered in sections \@ref(transport-zones) to \@ref(route-networks).
...
In fact we have already done all the hard work needed where to prioritize cycling infrastructure:
we have short but car-dependent commuting routes (generated from desire lines) created in section \@ref(routes), desire lines representing the first leg to public transport nodes created in section \@ref(nodes) and the ability to analyze route networks in terms of 'betweenness' and other conceptions for graph theory, covered in the previous section.
<!-- maybe it would be better to explain the code a little bit-->
