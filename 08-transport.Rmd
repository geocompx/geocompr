...
ways_road = opq(bbox = st_bbox(region_ttwa)) %>% 
...
res$osm_lines$highway = gsub("motorway|primary|secondary", "road", x = res$osm_lines$highway)
ways_bristol = res$osm_lines %>% 
  select(highway, maxspeed, ref)
ways_bristol$highway = as.factor(ways_bristol$highway)
ways_bristol = st_intersection(ways_bristol, region_ttwa)
