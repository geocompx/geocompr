...
region = getbb("Bristol", format_out = "sf_polygon") %>% 
  st_set_crs(4326)
region = st_sf(data.frame(Name = "Bristol (OSM)"), geometry = region$Bristol)
...
```{r zones, echo=FALSE, fig.cap="Regional boundaries (the thick black line represents the Travel to Work Area, the thinner black boundary represents the OSM boundary) for Bristol and the percentage of inhabitants travelling by car (left) and bicycle (right).", message=FALSE}
region_osm = readRDS("extdata/bristol-region.rds")
region_all = rbind(region_ttwa, region_osm)
zones$p_drive = zones$car_driver / zones$all * 100
zones$p_cycle = zones$bicycle / zones$all * 100
library(tmap)
tmap_mode("plot")
qtm(zones, c("p_drive", "p_cycle"), fill.breaks = c(0, 3, 6, 10, 30, 60, 100),
    fill.palette = viridis::plasma(n = 6)) +
  tm_shape(region_all) +
  tm_borders(lwd = c(5, 2), col = "black")
```
