...
The desert's extreme harh conditions and its remoteness spawned a truly unique and endemic flora and fauna.
...
In this chapter we would like to analyze the floristic composition by means of ordination techniques. 
Secondly, we would like to model the scores of the first ordination axis with environmental variables such as slope, catchment area, altitude and NDVI.
The resulting model shall serve to make spatial predictions of the floristic composition.
When doing so, we should account for over-optimistic predictions in the presence of spatial autocorrelation.

Ordinations are a popular tool in vegetation science to extract the main information from large species-plot matrices mostly filled with 0s. 
However, they are also used in remote sensing (spectral bands)
Principal component analysis (PCA) is probably the most famous ordination technique. 
It is a great tool to reduce dimensionality if one can expect linear relationships between variables, and if the non-existence of a variable in two plots (observations) can be considered a similarity.
In vegetation this is barely the case.
For one, relationships are usually non-linear along environmental gradients (humidity, altitude, soil, etc.). 
Secondly, the joint absence of a species in two plots is often hardly an indication for similarity.
Suppose we don't find a species in the driest (extreme desert) and the most moist locations (tree savannah) of our sampling.
Then we really should refrain from counting this as a simlilarity.
