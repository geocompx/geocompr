...
```{r tmshape, echo=FALSE, message=FALSE, fig.cap="New Zealand's shape plotted with fill (left), border (middle) and fill and border (right) layers added using tmap functions."}
...
```{r tmlayers, message=FALSE, fig.cap="Maps with additional layers added to the final map of Figure 9.1."}
...
```{r tmstatic, message=FALSE, fig.cap="The impact of changing commonly used fill and border aesthetics to fixed values."}
...
```{r tmcol, message=FALSE, fig.cap="Comparison of base (left) and tmap (right) handling of a numeric color field.", echo=FALSE, out.width="45%", fig.show='hold', warning=FALSE, message=FALSE}
plot(nz["Land_area"])
tm_shape(nz) + tm_fill(col = "Land_area")
```
...
- `palette` which defines the color scheme, for example `BuGn`.
...
```{r tmpal, message=FALSE, fig.cap="Illustration of settings that affect color settings. The results show (from left to right): default settings, manual breaks, n breaks, and the impact of changing the palette.", echo=FALSE, fig.asp=0.56}
...
```{r break-styles, message=FALSE, fig.cap="Illustration of different binning methods set using the style argument in tmap.", echo=FALSE}
...
```{r na-sb, message=FALSE, fig.cap="Map with additional elements - a north arrow and scale bar.", fig.asp=1}
map_nz + 
...
```{r layout1, message=FALSE, fig.cap="Layout options specified by (from left to right) title, scale, bg.color and frame arguments.", echo=FALSE, fig.asp=0.56}
...
```{r tmstyles, message=FALSE, fig.cap="Selected tmap styles: bw, classic, cobalt and col_blind (from left to right).", echo=FALSE, fig.asp=0.56}
...
```{r urban-facet, message=FALSE, fig.cap="Faceted map showing the top 30 largest 'urban agglomerations' from 1970 to 2030 based on population projects by the United Nations.", fig.asp=0.5}
...
```{r insetmap1, message=FALSE, fig.cap="Inset map providing a context - location of the central part of the Southern Alps in New Zealand."}
library(grid)
nz_height_map
...
Faceted maps, described in \@ref(faceted-maps), is can show how the spatial distribution of variables change (e.g. over time), but the approach has disadvantages.
...
```{r urban-animated, message=FALSE, fig.cap="Animated map showing the top 30 largest 'urban agglomerations' from 1950 to 2030 based on population projects by the United Nations. Animated version available online at geocompr.robinlovelace.net.", echo=FALSE}
...
```{r animus, echo=FALSE, message=FALSE, fig.cap="Animated map showing population growth and state formation and boundary changes in the United States, 1790-2010. Animated version available online at geocompr.robinlovelace.net."}
...
<!-- including the appearance of popup messages when users click or mouse-over geographic features and maps dynamically linked to non-geographic plots.^[ -->
...
```{r tmview, message=FALSE, fig.cap="Interactive map of New Zealand created with tmap in view mode. Interactive version available online at geocompr.robinlovelace.net.", echo=FALSE}
tmap_mode("view")
m_tmview = map_nz
...
```{r sync, message=FALSE, fig.cap="Faceted interactive maps of global coffee producing in 2016 and 2017 in 'sync', demonstrating tmap's view mode in action.", echo=FALSE}
...
Consider the following example where **sf** is used to intersect lines and polygons and then visualised with **mapview**.
...
<!-- The command `google_map(key = key) %>% add_polygons(st_transform(nz, 4326))` creates an interactive map of New Zealand, assuming `key` contains an API key. -->
<!-- Many other functions are provided by the package, providing an R interface to a wide range of mapping services including routing, traffic visualization and geocoding. -->
...
Its use of WebGL enables it to interactively visualize large datastes (up to millions of points).
The package uses
<!-- commented-out the below to reduce hyperlinks (RL) -->
<!-- [Mapbox](https://www.mapbox.com/) -->
Mapbox
[access tokens](https://www.mapbox.com/help/how-access-tokens-work/), which you must register for before using the package.
...
A unique feature of **mapdeck** is it's provision of interactive '2.5d' perspectives, illustrated in Figure \@ref(fig:mapdeck).
...
```{r, engine='zsh', echo=FALSE, eval=FALSE}
...
```{r mapdeck, echo=FALSE, fig.cap="Map generated by mapdeck, representing road traffic casualties across the UK. Height of 1 km cells represents number of crashes."}
...
```{r leaflet, message=FALSE, fig.cap="The leaflet package in action, showing cycle hire points in London."}
...
```{r lifeApp, echo=FALSE, message=FALSE, fig.cap="Screenshot showing minimal example of a web mapping application created with shiny."}
...
```{r coffeeApp, echo=FALSE, message=FALSE, fig.cap="coffeeApp, a simple web mapping application for exploring global coffee production in 2016 and 2017. Interactive version available online at geocompr.robinlovelace.net."}
...
```{r nz-plot, message=FALSE, fig.cap="Map of New Zealand created with plot(). The legend to the right refers to elevation (1000 m above sea level)."}
...
```{r nz-gg, message=FALSE, fig.cap="Map of New Zealand created with ggplot2."}
library(ggplot2)
...
knitr::kable(map_gpkg_df, caption = "Selected general-purpose mapping packages.", booktabs = TRUE) %>%
...
knitr::kable(map_spkg_df, caption = "Selected specific-purpose mapping packages, with associated metrics.", booktabs = TRUE)
```
...
```{r cartomap1, echo=FALSE, message=FALSE, fig.cap="Comparison of standard map (left) and continuous area cartogram (right)."}
...
```{r cartomap2, echo=FALSE, message=FALSE, fig.cap="Comparison of non-continuous area cartogram (top) and Dorling cartogram (bottom).", fig.asp=0.32}
