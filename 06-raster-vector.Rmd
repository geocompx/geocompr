...
The aggregated polygons of the `grain` dataset have rectilinear boundaries which arise from being defined by the boundaries of pixels.
The **smoothr** package described in Chapter \@ref(geometry-operations) can be used to smooth the edges of the polygons.
The smoothed polygons will not have the same exact spatial footprint as the original pixels, so caution should be used for using the smoothed polygons for further analysis.
