...
`gridded()` converts a `SpatialPolygonsDataFrame` into a `SpatialGridDataFrame` which in turn can be used as an input for **raster**'s `stack()` function.
...
Reclassify inhabitant data (see also section \@ref(local-operations)).
...
Aggregate it to a resolution of 20000 sq-km.
...
Please note that the unzipped file has a size of nearly 1.3 GB.
To read it in you can use `readr::read_csv`.
This takes 30 seconds on my machine (16 GB RAM)
However, `data.table::fread()` is faster (on my machine 12 seconds).
This will return an object of class `data.table()`.
