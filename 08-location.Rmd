...
```{r}
our_reclassify = function(x, y){
  reclassify(x = x, rcl = y, right = NA)
}
reclass = map2(as.list(input_ras), rcl, our_reclassify) %>% 
  stack()
...
The defined metropolitan areas (Fig. \@ref(fig:metro-areas); see also `code/08-location-jm.R`) suitable for bike shops are still missing a name.
...
However, if three or four attempts have already failed, this is a good indication that the requested information is indeed unavailable.
...
Overall, we are satisfied with the `locality` column serving as metropolitan names (Table \@ref(tab:metro-names)) apart from one exception, namely Velbert.
