...
Geomarketing (also called location analysis) is a broad field of research and (largely) commercial application, the aim of which is usually to decide the optimal location for new services.
...
animals have needs which can be best met in optimal locations based on variables that change over space (called gradients in the ecological literature --- see chapter xx).
<!-- add reference!! -->
...
Many store location analysis consultancies have developed to help people ask such questions, and they would happily charge high rates for their expertise.
It is testament to the rate of technological development that this questioned can now be answered by informed citizens using free and open source software.
The process of investigating it will demonstrate a number of geo-computational techniques learned during the first chapters of the book.
The rest of the book will demonstrate common steps in geomarketing, to find suitable locations for the cycle stores.
Note that although we have applied these steps to a specific case study, they could be generalised to many scenarios of store location or public service provision:

<!-- just internal structure and brain storming -->
- Demographic data access: download German census data and convert it into a raster (resolution: 1 km^2^).
- Identify potential host cities: find metropolitan areas (resolution: 10 km^2^).
- Access detailed geographic data for these areas: download OSM data for the potential host cities, identify existing cycle shops and other interesting points of interest and convert these to a raster grid.
...
metro_names = 
  mutate(metro_names,
         # replace Velbert
         locality = ifelse(locality == "Velbert", administrative_area_level_2, locality),
         # replace umlaut ü
...
For instance, the `opq()` function has retrieved a wrong or too small bounding box.
In any case, is is highly unlikely that there are no shops in any of our defined metropolitan areas.
...
To make sure that each list element (an sf data.frame) comes with the same columns, we only keep the `osm_id` and the `shop` columns with the help of another `lapply`-loop.
...
shops = lapply(shops, select, osm_id, shop)
...
`count` simply counts the number of spatial objects, here the shop points, falling into one raster cell, and returns this value as the output cell value.
...
poi = input$pop
poi[] = NA
poi = rasterize(x = shops, y = poi, field = "osm_id", fun = "count")
```
...
The final workstep, the calculation of a final score by summing up all prepared rasters, is a simple one-liner.
...
knitr::include_url("figures/08_bikeshops_berlin.html")
```
...
Thereby, we should account for the fact that the farther away the people are from the shop, the more unlikely it becomes that the actually visit it (distance decay function).
...
- We need to find suitable and affordable real estate (accessible, parking spots, frequency of passers-by, big windows, friendly staff, etc.).
