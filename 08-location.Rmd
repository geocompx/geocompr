...
The here presented approach is a typical example of the normative usage of a GIS.
...
Maybe it would be wiser to use only those which might be interesting for bike shops such as... .
- There is a finer resolution of the population raster (100 m).
Maybe data at a better resolution changes the output (see exercises).
- We have used only a limited set of variables. 
The INSPIRE database might contain much more data of possible interest to our analysis.
Additionally, the purchasing power or even better the retail purchasing power for bikes would further improve the obtained result.
...
So far we have merely identified areas, 1 by 1 km in size, of possible interest for a bike shop in accordance with our survey.
We could continue the analysis as follows:

- choose optimal location based on number of inhabitants -> you should reach as much people within 15 minutes as possible (catchment area routing)
