...
This is because one polygon just touches the corner of another polygon (western Germany, Cologne/Düsseldorf area; see Fig. \@ref(fig:metro-areas)).

```{r metro-areas, echo = FALSE, fig.width = 1, fig.height = 1, fig.cap = "The aggregated raster (resolution: 20 km) with the identified metropolitan areas (golden rectangles) and the corresponding names."}
...
Since `ggmap::revgeocode()` only accepts one coordinate at a time, we iterate over each coordinate of `coords` via a loop (`ldply()`).
...
We are also good cyberspace citizens by trying not to overburden the server with too many queries within a short amount of time. 
Instead we let the loop sleep between one and four seconds after each iterationbefore accessing the reverse geocoding API again.
