...
```{r census-stack, echo = FALSE, fig.cap = "Gridded German census data of 2011."}
knitr::include_graphics("figures/08_census_stack.png")
```


<!-- find out about new lines in headings + blank cells-->
We `reclassify()` the ordinal-scaled raster in accordance with our survey (see section \@(use-case)).
...
By contrast, we convert the remaining input rasaters into weight rasters whereas the highest weights are in correspondence with our survey.
...
rcl_women = matrix(c(1, 1, 3, 2, 2, 2, 3, 3, 1, 4, 4, 0, 5, 5, 0,
                     6, 6, 8000), ncol = 3, byrow = TRUE)
rcl_age = matrix(c(1, 1, 3, 2, 2, 0, 3, 3, 0, 4, 4, 0, 5, 5, 0,
                   6, 6, 8000), ncol = 3, byrow = TRUE)
rcl_hh = matrix(c(1, 1, 3, 2, 2, 2, 3, 3, 1, 4, 4, 0, 5, 5, 0,
                   6, 6, 8000), ncol = 3, byrow = TRUE)
rcl = list(rcl_pop, rcl_women, rcl_age, rcl_hh)
nms = names(input)
for (i in seq_len(nlayers(input))) {
  input[[i]] = reclassify(input[[i]], rcl = rcl[[i]], right = NA) 
}
names(input) = nms
```
...
Hence, we replace Velbert with the correponding name in the `administrative_area_level_2` column, that is Düsseldorf (Fig. \@ref(fig:metro-areas)).
...
Here, normative Nutzung von GIS.
