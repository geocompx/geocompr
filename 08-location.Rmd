...
One approach is to dissolve all polygons (see section \@ref(spatial-data-aggregation)).
...
metro_names = 
  ...
  mutate(# replace Velbert
         locality = ifelse(locality == "Velbert", administrative_area_level_2, 
                           locality),
         # replace umlaut ü
...
To make sure that each list element (an sf data.frame) comes with the same columns, we only keep the `osm_id` and the `shop` columns with the help of another `map`-loop (`lapply()` would do exactly the same here).
