...
They are a part of the **spDataLarge** package:
...
To summarize, these were: identifying short but car-dependent commuting routes (generated from desire lines) were created (section \@ref(routes)); creating desire lines representing the first leg to public transport nodes were created in (section \@ref(nodes)); and analyzing transport systems at the route network using concepts from graph theory (section \@ref(route-networks)).
...
This chapter provide a taster of the possibilities of using geocomputation for transport research.
...
By aggregating OpenStreetMap data using buffers and spatial data methods presented in Chapters \@ref(attr) and \@ref(spatial-operations), for example, it would be possible to detect the presence of green space in close proximity to parks.
