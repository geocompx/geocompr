...
- This chapter requires the following packages to be loaded, all but the last two of which have been used previously^[
**osmdata** and **nabor** must also be installed althoughs these packages do not need to be loaded.
]
...
We will use travel data stored in `bristol_od`, which was provided by the [ons.gov.uk](https://www.ons.gov.uk/help/localstatistics) data portal.
...
A simplified version of Figure \@ref(fig:zones) is created with the code below (see `07-zones.R` to reproduce the figure and section \@ref(faceted-maps) for details on facetted maps with **tmap**):
...
```{r zones, echo=FALSE, fig.cap="Number of trips (commuters) living and working in the region. The left map shows zone of origin of commute trips; the right map shows zone of destination (generated by the script 07-zones.R).", message=FALSE}
...
```{r, eval=FALSE}
plot(st_geometry(desire_lines), lwd = desire_lines$all / 500)
```
...
plot(st_geometry(desire_rail), expandBB = c(.1, .1, .1, .1))
