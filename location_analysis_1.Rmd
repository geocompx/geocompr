...
Plotting our polygons reveals eight regions.
Each region consists of one ore more polygons (pixels).
It would be nice if we could merge the pixels belonging to one region.
...
To name the found several metropolitan areas, we use a reverse geocoding approach.
Reverse gecoding finds the address for a specific coordinate
That is, we extract he centroid coordinate of each area, and send it to a reverse geocoding API.
Re

```{r}
# reverse geocoding to find out about the metropolitan names
polys_2 = st_transform(polys, 4326)
# after the aggregation st_coordinates no longer works...
# coords = st_coordinates(polys_2)
coords = coordinates(as(polys_2, "Spatial"))
