...
plot(inh_agg > 500000)
polys = raster::rasterToPolygons(inh_agg[inh_agg > 500000, drop = FALSE])
# dissolve on spatial neighborhood
# GO ON FROM HERE TOMORROW-------------
nbs = over(polys, polys, returnList = TRUE)
polys = spTransform(polys, "+proj=longlat +datum=WGS84 +ellps=WGS84 +towgs84=0,0,0")
...
library(osmdata)
q0 <- opq('Dresden')
q1 <- add_osm_feature(q0, key = 'highway', value = 'cycleway')
cway_sev <- osmdata_sp(q1)


metros = c("Munich", "Stuttgart", "Nuremberg", "Frankfurt", "Cologne",
