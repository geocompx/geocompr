...
```{r providers, eval=qgisprocess_installed}
qgis_providers()
#> # A tibble: 6 × 2
...
```{r uniondata, echo=FALSE, fig.cap="Illustration of two areal units: incongruent (black lines) and aggregating zones (red borders).", eval=qgisprocess_installed}
library(tmap)
...
```{r 09-gis-6, eval=qgisprocess_installed}
alg = "native:union"
qgis_arguments(alg) |>
...
```{r 09-gis-7, eval=qgisprocess_installed}
...
```{r 09-gis-7c, eval=qgisprocess_installed}
...
```{r sliver, echo=FALSE, fig.cap="Sliver polygons colored in red (left panel). Cleaned polygons (right panel).", eval=qgisprocess_installed}
