...
if ("qgisprocess" %in% installed.packages()) {
  # Check that QGIS and required plugins are available to prevent errors in subsequent checks:
  has_qgis_plugins = 
    qgisprocess::has_qgis() && 
    c("grassprovider", "processing_saga_nextgen") %in% 
    qgisprocess::qgis_plugins()$name |>
    all()
}
```
...
You can do so from within QGIS with the [Plugin Manager](https://docs.qgis.org/3.28/en/docs/training_manual/qgis_plugins/fetching_plugins.html) or programmatically with the help of the Python package [qgis-plugin-manager](https://github.com/3liz/qgis-plugin-manager) (at least on Linux).
