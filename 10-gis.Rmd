...
The final `.quiet` argument only tells **qgisprocess** to be less verbose.
...
clean = qgis_run_algorithm("grass7:v.clean", input = union_sf,
                           tool = 10, threshold = 25000, .quiet = TRUE)
...
The **terra** package's `terrain()` command allows the calculation of several fundamental topographic characteristics such as slope, aspect, TPI (*Topographic Position Index*), TRI (*Topographic Ruggedness Index*), roughness, and flow directions.
It allows choosing a terrain characteristic via th `v` argument and, in the case of `"slope"` and `"aspect"`, also the output unit.
...
It returns a new raster object with the same dimensions as the original one but with new values representing the selected terrain characteristic.
