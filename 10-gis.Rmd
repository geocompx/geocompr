...
The GRASS GIS provider in QGIS was called `grass7` before QGIS version 3.36.
If you have an older QGIS version, you must prefix the algorithms with `grass7` instead of `grass`.
```
...
`r.topidx` is a thin wrapper -- it takes only an elevation raster as input and writes a topographic-index raster as output, as we can see in its help.
