...
Lines of code that do not contain valid R should be commented on to prevent errors, as with line 1 of the `10-hello.R` script.
]
...
- Comment on the script sufficiently for others (and your future self) to understand it, but not too much. At a minimum, a comment should state the purpose of the script (see Figure \@ref(fig:codecheck)) and (for long scripts) divide it into sections. This can be done in RStudio, for example, with the shortcut `Ctrl+Shift+R`, which creates 'foldable' code section headings.
...
Note that the functions we created are called recursively in `lapply()` and `vapply()` function calls.
]
...
Not only have we discussed them in the abstract, but also we have created working examples of each to solve a specific problem:
...
1. In Section \@ref(geometric-algorithms) we calculated that the area and geographic centroid of the polygon represented by `poly_mat` was `r sum(A)` and `r round(c(weighted.mean(C[, 1], A), weighted.mean(C[, 2], A)), 1)`, respectively.
...
1. It was stated that the algorithm we created only works for *convex hulls*. Define convex hulls (see Chapter \@ref(geometric-operations)) and test the algorithm on a polygon that is *not* a convex hull.
     - Bonus 1: Think about why the method only works for convex hulls and note changes that would need to be made to the algorithm for other types of shapes to be calculated.
