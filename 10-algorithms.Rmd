...
```{r codecheck, echo=FALSE, fig.cap="Illustration of 'code checking' in RStudio. This example, from the script 10-centroid-alg.R, highlights an unclosed curly bracket on line 19.", fig.scap="Illustration of 'code checking' in RStudio."}
...
- The individual steps that allowed this script to work were described as algorithm, a computational recipe.
- To generalize the algorithm we converted into modular functions which were eventually combined to create the function `poly_centroid()` in the previous section.
...
In its current state, it does not even work on most (non-convex) polygons!
A question arising from this is: how would one generalise the function.
Two options are 1) to find a way to create triangular meshes of non-convex polygons (a topic covered in the online [algorithm](https://geocompr.github.io/geocompkg/articles/algorithm.html) article that supports this chapter) and 2) to explore other centroid algorithms that do not rely on triangular meshes.

A wider question is: is it worth programming a solution at all when high performance algorithms have already been implemented in packaged functions such as `st_centroid()`?
...
We cannot gurarntee that, having read this chapter, you will be able to rapidly create new functions for your work.
