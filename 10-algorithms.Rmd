...
<!-- JM: I think the quote refers to something else, e.g., humans can easily recognize faces or can easily take a step, something which is very hard for a computer/roboter. centroid computation might not be the most complex procedure in the world, still, when asking your students how to do so you will find that they will not find it as easy as recognizing a friend -->
...
<!-- JM: the reprex package is great especially for communicating on Github, via twitter, etc. but can you really use it to test if your own code is reproducible?-->
...
Geoalgorithms, such as those we have encountered in Chapter \@ref(gis), are algorithms that take geographic data in and, generally, return geographic results (alternative terms for the same thing include *GIS algorithms* and *geometric algorithms*).
That may sound simple but it is a deep subject with an entire academic field --- *Computational Geometry*, a branch of computer science --- dedicated to their study [@berg_computational_2008]. <!-- JM: maybe, we should add here GIScience and Geomatics?-->
...
For the purpose of this section, we choose an approach that is easy to visualize: breaking the polygon into many triangles and finding the centroid of each of these, an approach discussed by @kaiser_algorithms_1993 alongside other centroid algorithms.
...
<!-- JM: I wouldn't expect the reader to remember all four steps, hence, I would repeat them-->
<!-- JM: This is not a generic way of finding all triangles, or if it is then this needs to be explained in more detail. We could also think about using st_triangulate, I guess it is easier for the reader to understand. But this is just a suggestion, if you want to do it from scratch, however, more explanation is needed, I suppose. If you do not want to use sf-functionality, consider the following:

```{r}
library(geometry)
ind = delaunayn(poly_mat)
# here I just repeat the first column, to be able to plot triangles (only necessary for visualization purposes)
ind = cbind(ind, ind[, 1])
plot(poly_mat, type = "l")
for (i in seq_len(nrow(ind))) {
  lines(poly_mat[ind[i, ], ], col = i)
}
```
-->
...
<!-- C1 = colMeans(T1[-4, ])  # easier to understand, also I wouldn't put the formula in the text but say that the centroid of a triangle is simply the mean of the x and the y coordinates. If you don't use sf-power, then there is no need for repeating the origin, which would only be needed if you wanted to convert the triangle into a sfg polygon. Skipping the unnecessary fourth coordinate would also have the advantage that you could write much of the remaining code without indices. Would make the code more generic, more concise and better to read -->
```{r}
...
<!-- I think this is way too complicated. These are rectangular triangles, so you could use 1/2 * a * b. But ok, then you would have to compute the triangle sides..., ok, maybe, your approach is better though it seems too be complicated.
Wouldn't it be easier just to use `st_area`, I mean, nobody says that we cannot use already existing functions, and this way we could spare us the effort of explaining how to compute the area of a triangle (same suggestion for finding the triangles, see above), and leave it as an exercise to the reader to do it manually. You could also use geometry::polyarea(T1[, 1], T1[, 2]) if you don't want to use sf functionality. -->
...
<!-- I guess the code chunk requires much more explanation. What is i? Why -2? Why do you use O instead of just 1? Since it is a large code chunk it would also benefit from a few comments -->
```{r}
i = 2:(nrow(poly_mat) - 2)
...
<!-- Here, I would prefer the manual computation over the weighted.means command because this way it is clearer what we are doing. Since it is vectorized, it is also concise code:
```{r}
# compute the single weights
weights = A / sum(A)
# compute the centroid using the weights
colSums(C * weights)
```
-->
...
<!-- Assuming the function only expects three coordinates, I would just write:
x should be a dataframe or matrix with two columns representing x- and y coordinates and three rows
```{r}
t_centroid = function(x) {
  if (nrow(x) > 3 | nrow(x) < 3) {
    stop("x must have three rows") 
  }
  colMeans(x)
}
t_centroid(poly[-4, ])
