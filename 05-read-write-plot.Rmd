...
Geocomputation usually involves reading existing data, and saving it after some processing.
...
This if vital if you would like to encourage evidence-based policies.
...
Exchanging spatial data between different software packages each coming with its own format was troublesome in the beginnging.

GDAL ^[prounounce "goo-dal", with the double making a reference to object-orientation]put an end to these troubles by enabling <!--name and pronounciation--> reading and writing many raster and vector data formats.
...
1. Download the global monthly minimum temperature with a temporal resolution of five minutes using the **raster** package.
