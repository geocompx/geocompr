...
A key feature of some advanced routing engines (both locally hosted and remotely hosted) such as R5, OpenTripPlanner and [Valhalla](https://github.com/valhalla/valhalla) (which at the time of writing does not have an R interface to the best of our knowledge) is the provision of *multi-modal* routes: allowing a single journey from A to B to be made by more than one mode of transport, such as cycling from home to a bus stop, catching a bus to the city centre, and then walking to the destination.
...
Route level outputs contain a single feature (typically a multilinestring and associated row in the data frame representation) per origin-destination pair.
Most routing engines return route level by default; multi-modal engines can provide outputs at the 'leg' level (one feature per continuous movement by a single mode of transport) while the **cyclestreets** package, for example, provides segment level data by default.
...
We will filter-out a subset of the desire lines with a focus on estimating cycling potential based on the observation that the benefits cycling trips are greatest when they replace car trips and that relatively short trips are more likely to be cycled than long trips [@lovelace_propensity_2017].
...
saveRDS(routes_short, "routes_short.Rds")
gh_release_upload = function(file, tag = "0.0.1") {
  msg = glue::glue("gh release upload {tag} {file} --clobber")
  message("Running this command:\n", msg)
  # processx::run(msg, spinner = TRUE) # fails...
  system(msg)
}
# piggyback::pb_upload("routes_short.Rds") # fails with:
# Error in parse_url(url) : length(url) == 1 is not TRUE
# piggyback::pb_download_url("routes_short.Rds")
gh_release_upload("routes_short.Rds", tag = "1.9.1-2")
```

```{r 13-transport-18-2, echo=FALSE}
u = "https://github.com/Robinlovelace/geocompr/releases/download/1.9.1-2/routes_short.Rds"
routes_short = readRDS(url(u))
```
...
Not that the red routes and black desire lines do not start at exactly the same points.
...
```{r routes, warning=FALSE, fig.cap="Routes along which many (100+) short (<5km Euclidean distance) car journeys are made (red) overlaying desire lines representing the same trips (black) and zone centroids (dots).", fig.scap="Routes along which many car journeys are made."}
...
tm_shape(desire_rail, bbox = bb) + tm_lines() +
  tm_shape(desire_rail$leg_orig) + tm_lines(col = "red", lwd = 3) +
  tm_shape(desire_rail$leg_via) + tm_lines(col = "gray", lwd = 2) +
  tm_shape(bristol_stations) + tm_dots(col = "red") +
  tm_shape(desire_rail$leg_dest) + tm_lines(col = "blue", lwd = 5) +
...
To be specific: any routing engines requires some kind of route network input, so any transport research involving routes must invariably involve route network data in the internal or external routing engines (in the latter case the route network data is not necessarily imported into R).
However, route networks are also important outputs in many transport research projects: summarising data such as the potential number of trips made on particular segments and represented as a route network, can help prioritize investment where it is most needed.
\index{network}
...
Imagine that 50% of car trips between 0 to 3 km in route distance are replace by cycling, a percentage that drops by 10 percentage points for every additional km of route distance so that 20% of car trips of 6 km are replaced by cycling and no car trips that are 8 km or longer are replaced by cycling.
...
The outputs of the two preceeding code chunks are summarised in Figure \@ref(fig:rnetvis) below.
...
tm_shape(route_network_scenario) +
  tm_lines(lwd = "bicycle", scale = 9, title.lwd = "N. bike trips (modelled, one direction)")
```
...
The above code chunk summarized a simple feature\index{sf} object representing around 3,000 segments on the transport network\index{network}.
...
To overcome this issue functionality was developed in the
**stplanr**\index{stplanr (package)}
and subsequently
**sfnetworks**\index{sfnetworks (package)}
packages to represent route networks simultaneously as graphs *and* a set of geographic lines.
We will demonstrate **sfnetworks** functionality on the `bristol_ways` object.

```{r 13-transport-23}
library(sfnetworks)
bristol_ways$geometry_type = st_geometry_type(bristol_ways)
ways_freeway = bristol_ways
ways_sfn = as_sfnetwork(ways_freeway)
class(ways_sfn)
ways_sfn
```
...
ways_centrality = ways_sfn |> 
  ...
  mutate(  
    lengths = st_length(.),  
    betweenness = tidygraph::centrality_edge_betweenness(lengths)  
    ) 
tm_shape(ways_centrality |> st_as_sf()) +
  tm_lines(lwd = "betweenness", scale = 9) +
  tm_shape(route_network_scenario) +
  tm_lines(lwd = "bicycle", scale = 9, title.lwd = "N. bike trips (modelled, one direction)", col = "green")
```
...
```{r 13-transport-25-2, echo=FALSE}
...
```{r 13-transport-27, eval=FALSE, echo=FALSE}
n_trips = route_cycleway$all / 100
tm_shape(route_cycleway) +
  tm_lines("distance", lwd = "n_trips")
```
