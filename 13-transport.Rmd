...
However, 5 km Euclidean distance (or around 6-8 km of route distance) can realistically be cycled by many people, especially if they are riding an [electric bike](https://www.sciencedirect.com/science/article/pii/S0967070X21003401).
<!-- Todo: add reference (RL 2022-07) -->
We will therefore only route desire lines\index{desire lines} along which a high (100+) number of car trips take place that are 5 km in distance or less:
...
desire_lines_short = desire_lines %>% 
  filter(car_driver >= 100 & distance_km <= 5)
```

In the code above `st_length()` calculated the length of each desire line, as described in Section \@ref(distance-relations).
The `filter()` function from **dplyr** was used to filter the `desire_lines` dataset based on the criteria outlines above\index{filter operation|see{attribute!subsetting}}, as described in Section \@ref(vector-attribute-subsetting).
The next stage is to convert these desire lines into routes.
We will do this using the publicly available OSRM service \index{routing} with the **stplanr** functions `route()` and `route_osrm()`\index{stplanr (package)}.
The code chunk below creates `routes_short`, an `sf` object representing routes on the transport network\index{network} that are suitable for cycling (according to the OSRM routing engine at least), one for each desire line:
...
We could keep the new `routes_short` object separate from the straight line representation of the same trip in `desire_lines_short` but, from a data management perspective, it makes more sense to combine them: they represent the same trip.
...
```{r 13-transport-19}
desire_lines_short$geom_car = st_geometry(routes_short)
```

This allows plotting the desire lines\index{desire lines} along which many short car journeys take place alongside likely routes traveled by cars by referring to each geometry column separately (`desire_lines_short$geometry` and `desire_lines_short$geom_car` in this case).
...
plot(st_geometry(desire_lines_short))
plot(desire_lines_short$geom_car, col = "red", add = TRUE)
plot(st_geometry(st_centroid(zones_od)), add = TRUE)
```

\index{routes}

Plotting the results on an interactive map, with `mapview::mapview(desire_lines_short$geom_car)` for example, shows that many short car trips take place in and around Bradley Stoke.
