...
A key feature of some advanced routing engines (both locally hosted and remotely hosted) such as R5 and OpenTripPlanner is the provision of *multi-modal* routes: allowing a single journey from A to B to be made by more than one mode of transport, such as cycling from home to a bus stop, catching a bus to the city centre, and then walking to the destination.
...
The package **stplanr** aims to tackle this problem by providing a unified interface for generating routes with the `route()` function.
