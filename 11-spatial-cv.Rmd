...
Packages **lgr**, **kernlab**, **ml3measures**, **paradox**, **pROC**, and **spDataLarge** must also be installed although these do not need to be attached.
]
...
```{r 11-spatial-cv-2}
data("lsl", "ta", "study_area", package = "spDataLarge")
```

This should load three objects: a `data.frame` named `lsl`, a `RasterStack` (see Section \@ref(raster-classes)) named `ta` containing terrain attribute rasters, and an `sf` object named `study_area`.
...
Computing these terrain attribute rasters from the digital elevation model (`ta$elev`) with the help of R-GIS bridges (see Chapter \@ref(gis)) and extracting the corresponding values to our landslide/non-landslide data frame might be a nice exercise for the interested reader (see exercises).
...
```{r building-blocks, echo=FALSE, fig.height=4, fig.width=4, fig.cap="Basic building blocks of the mlr3 package. Source: @becker_mlr3_2021 (Permission to reuse this figure was kindly granted.)", fig.scap="Basic building blocks of the mlr3 package."}
...
Since the response is binary (two-category variable) and has a spatial dimension, we create a classification\index{classification} task with `mlr3spatiotempcv::TaskClassifST$new()` (for non-spatial tasks, use `TaskClassif$new()` or `TaskRegr$new()` for regression\index{regression} tasks, see `?Task` for other task types).^[The **mlr3** ecosystem makes heavily use of **R6** classes. And though you might use **mlr3** without knowing the specifics of **R6**, it might be rather helpful. To learn more about **R6**, we recommend [Chapter 14](https://adv-r.hadley.nz/fp.html) of the Advanced R book [@wickham_advanced_2019].]
...
Please note that `TaskClassifST$new()` also accepts an `sf`-object as input for the `backend` parameter.
...
Most importantly, (spatial) resampling in **mlr3spatiotempc** is straightforward, requiring only two more steps: specifying a resampling method and running it.
...
This takes some time (around 16 seconds on a modern laptop) because it computes the performance measure for 500 models. 
...
The output of the preceding code chunk is a bias-reduced assessment of the model's predictive performance (here given as AUROC), as illustrated in the following code chunk (required input data is saved in the file `11-sp_cv.rds` in the book's GitHub repo):

```{r 11-spatial-cv-21, echo=FALSE}
sp_cv = readRDS("extdata/11-sp_cv.rds")
conv_cv = readRDS("extdata/11-conv_cv.rds")
```

```{r 11-spatial-cv-22}
# summary statistics of the 500 models
sp_cv$score(measures = msr("classif.auc"))[, summary(classif.auc)]
# mean AUROC of the 500 models
sp_cv$agg(measures = msr("classif.auc"))
```
...
boxplot(sp_cv$score(measures = msr("classif.auc"))[["classif.auc"]],
...
However, using more than 50 iterations in the random search would probably yield hyperparameters\index{hyperparameter} that result in models with a better AUROC [@schratz_hyperparameter_2019].
...
For a discussion on the need for (spatial) tuning of hyperparameters, please refer to @schratz_hyperparameter_2019.
