...
In case you have run the GRASS example from the previous section before, please restart R before running the subsequent code chunk (RStudio shortcut: Crtl+Shift+F10).
...
Both polygon datasets are available in the **spData** package.
...
First, we will union the polygons.
For this, we need to come up with the name of the function in QGIS. 
`find_algorithms` lets you search all QGIS geoalgorithms with the help of regular expressions.
Here, we assume that the short description of the function contains the word "union".
...
Now that we know the name of the function (`qgis:union`), we wonder how we can use it.
...
union = run_qgis(alg, INPUT = incongruent, INPUT2 = aggregating_zones, 
                 OUTPUT = file.path(tempdir(), "cycle.shp"),
                 load_output = TRUE)
```
