...
#>Union (native:union)
#>
#>This algorithm checks overlaps ...
#>...
```
...
<!-- ok, this cannot work since one has to manually select the polygons which should be joined to other polygons...
ask community if there is still a qgis:eliminatesliverpolygons function
-->

```{r}
clean = run_qgis("grass7:v.clean",
         input = file.path(tempdir(), "single.shp"),
         tool = 10,
         threshold = 25000,
...
```{r, eval=FALSE}
clean = run_qgis("qgis:eliminateselectedpolygons",
                 INPUT = file.path(tempdir(), "single.shp"),
                 MODE = 2,
                 OUTPUT = file.path(tempdir(), "clean.shp"),
                 load_output = TRUE)
...
```{r grass-mapview, fig.cap="Shortest route between 24 cycle hire station on the OSM street network of London.", fig.scap="Shortest route between 24 cycle hire station.", echo=FALSE, out.width="80%"}
...
```{r postgis, echo=FALSE, fig.cap="Visualization of the output of previous PostGIS commands showing the highway (black line), a buffer (light yellow) and three restaurants (lightblue points) within the buffer.", fig.scap="Visualization of the output of previous PostGIS commands.", out.width="60%"}
