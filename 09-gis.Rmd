...
you *can* interact with GRASS (the longest-standing GIS CLI), QGIS, SAGA and gvSIG with dockable CLIs, but 'pointing and clicking' is the norm.
...
```{r saga-twi, fig.cap="SAGA wetness index of Mount Mongón, Peru.", echo=FALSE, out.width="70%"}
...
We emphasize these bridges because dedicated GIS software is well-known and a common 'way in' to understanding spatial data, and also because they provide access to many geoalgorithms.
It is important to be aware of other R bridges, to geospatial programs which based on command-line and are typically accessed via other programs through application programming interfaces (APIs).
These 'other bridges' include interfaces to spatial libraries (section \@ref(gdal) covers R interfaces to GDAL), spatial databases (section \@ref(postgis)) and web mapping services (see chapter \@ref(adv-map)).
This section provides only a snippet of what is possible with other bridges and, because of R's flexibility, many other bridges are possible.
...
`linkGDAL()` searches the computer for a working GDAL installation and adds the location of the executable files to the PATH variable, allowing GDAL to be called on different operating systems.
In the example below `ogrinfo` provides metadata above a vector dataset:
...
TauDEM (http://hydrology.usu.edu/taudem/taudem5/index.html) and the Orfeo Toolbox (https://www.orfeo-toolbox.org/) are other low level spatial data processing libraries offering a command line interface. 
...
In any case, the above example shows how to access low level libraries from the system, creating the possibility for new R bridges to these libraries to be created.
...
Furthermore, **sf** brings most of the power provided by GDAL, GEOS and PROJ to R via the R/C++ interface provided by **Rcpp**, which circumvents the command-line.
...
Databases enable storing and querying large datasets efficiently based spatial and non-spatial fields, and provide multi-user access and topology support.
...
```{r postgis, echo=FALSE, fig.cap="Visualization of the output of previous PostGIS commands showing the highway (black line), a buffer (light yellow) and three restaurants (lightblue points) within the buffer.", out.width="40%"}
