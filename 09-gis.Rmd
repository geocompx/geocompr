...
But GDAL offers more than I/O functions, e.g., reprojection, mosaicking and SQL tools. <!-- -> hint to GDAL help pages.-->
One can easily access these tools via the command line. 
In the code chunk below `linkGDAL()` searches the computer for a working GDAL installation and adds the executable to the PATH variable. 
...
Still, it shows how we can use GDAL via the command-line from within R indepedently of other packages, and nothing stops you from using much more advanced GDAL/OGR utilities.
Note also that the **RSAGA** package also uses the command line interface to use SAGA geoalgorithms from within R (see section \@ref(rsaga)). 
TauDEM and the Orfeo Toolbox <!-- check if this is true for Orfeo --> also offer a command line interface. 
As far as we know there is no R package <!-- check! --> providing the corresponding interface.
Yet, now you have the tools to build it yourself.
