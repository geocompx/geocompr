...
In this subsection, we will have a short glance at interfaces to spatial libraries, spatial databases and web mapping services.
This collection is by no means compreshensive.
Rather its intention is to give a wider look, an impression of what else is available.

As mentioned in the beginning, R is not a (spatial) database management system.
However, we can easily access, modify and create data stored in databases from within R.
The most important open-source spatial database is surely PostGIS, which is the spatial extension of PostgreSQL [@obe_postgis_2015].^[SQlite and its spatial extension Spatialite is surely also important but implicitly we have already presented this approach since GRASS is using SQlite in the background.]
PostGIS is a fully-fledged spatial database management system.
It is a great way to store and relate your spatial and non-spatial data in an ordered manner in one or more database schemas.
It is quite useful if your data becomes big which geographic data tends to be.
...
As a simple example we query here data we have collected and used for a review on qualitative GIS.
<!-- add Muenchow reference -->
This should only give you an idea how to work R and PostGIS in tandem.
...
# 2. make main_qual_gis a spatial point dataframe
