...
score_glm = sp_cv$score(measure = mlr3::measure("classif.auc")) %>%
  # keep only the columns you need
  .[, .(task_id, learner_id, resampling_id, classif.auc)]
```

The output of the preceding code chunk is a bias-reduced assessment of the model's predictive performance, as illustrated in the following code chunk (required input data is saved in the file `extdata/12-bmr_score.rds` in the book's GitHub repo).
...
score_glm = score[learner_id == "classif.log_reg"]
```

```{r 12-spatial-cv-22, eval=FALSE}
# compute the mean AUROC by resampling strategy
score_glm[, .(mean_auc = round(mean(classif.auc), 2)), by = resampling_id]
```
...
score_svm = result$score(measure = mlr3::msr("classif.auc")) %>%
  # keep only the columns you need
  .[, .(task_id, learner_id, resampling_id, classif.auc)]
```
...
```{r 12-spatial-cv-31}
score = readRDS("extdata/12-bmr_score.rds")
score_svm = score[learner_id == "classif.ksvm.tuned" & 
...
```{r 12-spatial-cv-33}
# final mean AUROC
round(mean(score_svm$classif.auc), 2)
```
