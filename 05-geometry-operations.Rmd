...
sfs_st_cast = read.csv("extdata/sfs-st-cast.csv") %>% 
  select(X, POINT, MULTIPOINT, LINESTRING, MULTILINESTRING, POLYGON, MULTIPOLYGON, GEOMETRYCOLLECTION)
names(sfs_st_cast)[1] = ""
knitr::kable(sfs_st_cast, caption = "Geometry casting on simple features objects. The first column represents input objects (see Section 2.1.5.1). The next columns represent an output geometry type. NA indicates that geometry casting is impossible. A number of elements in each object is shown in parentheses.")
```
