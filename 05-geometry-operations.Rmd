...
plot(extend(elev, 1), col = NA, legend = FALSE, mar = c(3, 3, 2, 4))
plot(elev_agg, add = TRUE)
xy_1 = xyFromCell(elev, 1)
xy_2 = xyFromCell(elev_agg, c(1, 2, 4, 5))
arrows(xy_2[, 1], xy_2[, 2], xy_1[, 1], xy_1[, 2], length = 0.05)
# points(xy_1, pch = 16, col = "black", cex = 2)
points(xy_2, pch = 21, bg = "salmon", col = "black")
plot(as.polygons(elev, dissolve = FALSE)[1], add = TRUE)
```
...
```{r pointextr, echo=FALSE, message=FALSE, warning=FALSE, fig.cap="Locations of points used for raster extraction (left) and their 1km buffers (right).", fig.asp=0.57}
...
zion_transect2 = cbind(c(-113.2, -112.9, -113.2), c(37.45, 37.2, 37.5)) %>%
  st_linestring() %>% 
  st_sfc(crs = crs(srtm)) %>% 
  st_sf()
...
plot_transect = ggplot(transect, aes(dist, srtm)) + 
  geom_line() +
...
rast_poly_nlcd = tm_shape(nlcd) +
  tm_raster(title = "Land cover", legend.show = TRUE) + 
  tm_shape(zion) +
  tm_polygons(lwd = 2, alpha = 0.3) +
...
res = knitr::knit_child('_05-ex.Rmd', quiet = TRUE, 
