...
It is a hierarchical data model that simplifies geographic data by condensing the complex range of possible geographic forms (e.g., line, point, polygon, multipolygon forms) into a single 'meta-class' called Geometry (Figure \@ref(fig:sf-ogc)).
...
However **sf** should be used with caution.
A major disadvantage is that it is not yet *feature complete* and unsupported by many spatial data analysis packages which still depend on **sp** classes and will likely to so for many years to come.
**sf** represents a step-change in R's handling of spatial data.
For that reason it is likely that many packages *will never transition to **sf** * so it is worth at least being aware of **sp** classes, even if you rarely use them for everyday geospatial tasks.

It is easy to translate between **sf** and **sf**: 
