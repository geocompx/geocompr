...
London, for example, can be represented by the coordinates `c(-0.1, 55.5)`, meaning it's located at -0.1 degrees longitude and 55.5 degrees latitude in a geographic ('lon/lat') CRS such as WGS 84 (EPSG: 4326).
The same point could also be represented in a projected CRS (EPSG: 27700) with 'Easting/Northing' values of `c(520e3, 625e3)`.
This means London is located is 520 km *East* and 625 km *West* of the $origin$, at -2° East and 49° North.


```{r}
p = st_point(c(-0.1, 55.5)) %>% 
  st_sfc() %>% 
  st_sf(crs = 4326)
st_transform(p, 27700)
```
