...
**sfheaders** is an R package that is designed to speed-up the construction of `sf` objects.  It's design philosophy enables you to

1. **build** **sf** objects from vectors, matrices and data.frames, rapidly, and without depending on the **sf** library
...
Although developed separately from **sf**, it is designed to be aligned with and create valid `sf` objects, of the type described in preceding sections.
...
```{r sfheaders-sfc_point2}
sfheaders::sfc_point(obj = v)
```

```{r sfheaders-sfc_linestring2}
sfheaders::sfc_linestring(obj = m)
```

```{r sfheaders-sfc_polygon2}
