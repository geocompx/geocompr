...
For users of Ubuntu 18.04 the steps are covered in detail in an article on [thinkr.fr](https://rtask.thinkr.fr/blog/installation-of-r-3-5-on-ubuntu-18-04-lts-and-tips-for-spatial-packages/).
```
...
```{r sf-ogc, fig.cap="The subset of the Simple Features class hierarchy supported by sf.", out.width="60%", echo=FALSE}
...
```{r contpop, fig.cap="Country continents (represented by fill color) and 2015 populations (represented by circles, with area proportional to population).", echo=FALSE, warning=FALSE}
...
This is used to plot India in the context of its giant Asian neighbors, with an emphasis on China to the East, in the following code chunk (which generates Figure \@ref(fig:china)):
...
```{r china, fig.cap="India in context, demonstrating how the expandBB argument can be used to control zoom in static plots.", warning=FALSE, echo=FALSE, out.width="50%"}
...
```{r multis, echo=FALSE, fig.cap="Illustration of multipoint, mutlilinestring and multipolygon geometries."}
old_par = par(mfrow = c(1, 3), pty = "s", mar = c(0, 3, 1, 0))
...
```{r geomcollection, echo=FALSE, fig.asp=1, fig.cap="Illustration of geometry collection.", out.width="33%"}
...
We therefore suggest converting `sf` objects to an intermediate `Spatial` class (from the **sp** package) for compatibility between raster and vector data, for example `crop(raster_obj, as(sf_object, "Spatial")` (see section \@ref(raster-vector)).
```
...
```{r vector-crs, echo=FALSE, fig.cap="Examples of geographic (WGS 84; left) and projected (NAD83 / UTM zone 12N; right) and coordinate systems for a vector data type.", message=FALSE, fig.asp=0.56}
...
```{r raster-crs, echo=FALSE, fig.cap="Examples of geographic (WGS 84; left) and projected (NAD83 / UTM zone 12N; right) and coordinate systems for a raster data type.", message=FALSE, fig.asp=0.56}
...
1. Run the code that 'generated' the map of the world in Figure \@ref(fig:contpop) at the end of \@ref(base-args). Find 2 similarities and 2 differences between the image on your computer and that in the book.
