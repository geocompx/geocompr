...
```{block2, type='rmdnote'}
On Mac and Linux operating systems a few requirements must be met, as described in the **sf** [README](https://github.com/r-spatial/sf) file for details.
```
...
Most point geometries contain only two dimension (3 dimensional CRSs contain an additional $z$ value, typically representing height about sea level).

In this system London, for example, can be represented by the coordinates `c(-0.1, 51.5)`, meaning it's located at -0.1 degrees east and 55.5 degrees north of the origin in a geographic ('lon/lat') CRS, as illustrated in the left of Figure \@ref(vectorplots)).
The same point could also be represented in a projected CRS with 'Easting/Northing' values of `c(532e3, 180e3)` (using *scientific notation* in which `1e2` means 100, `1e3` means 1,000) in the British National Grid (BNG).
In the vector data model this suggests that London is located 532 km *East* and 180 km *North* of the $origin$ of the BNG.
This can be verified visually: slightly more than 5 'boxes' 100 km in width separate the point representing London from the origin in Figure \@ref(vectorplots).
This shows that the origin of the BNG is located in the sea to the southeast of the UK, ensuring that all locations in the UK can be represented with positive Easting (East of the origin point) and Northing (North of the origin) values.
CRSs are described in section \@ref(coordinate-reference-systems).
...
A broader advantage is that the simple features data model is well-supported by other software products including PostGIS, which heavily influenced the design of **sf**.
Some spatial packages (including **tmap**, **mapview** and **tidycensus**) have added support for  **sf** but it will take many years for many packages to transition, and  some packages will never switch.
Fortunately these can still be used on `sf` objects, by converting them to the `Spatial` class used in **sp**: 
...
Conversely `Spatial` objects can be converted back to `sf` in the same way (or with `st_as_sf()`):
