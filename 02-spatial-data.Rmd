...
```{block2 type="rmdnote"}
Don't confuse geographic vector data with `vector`s in R.
The former is a data model, the latter is an R class just like `data.frame` and `matrix`.
There is a link between the two meanings, however: the spatial coordinates at the heart of the geographic vector data model can be represented in R using `vector` objects or lists of them.
```
