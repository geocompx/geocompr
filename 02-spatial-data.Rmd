...
Over the last two decades, several packages packages for reading and processing raster datasets has been developed.
As outlined in Section \@ref(the-history-of-r-spatial), chief among them was the **raster**, which was the premier package in the space until the development of **terra** and **stars**.
Both provide powerful and performant functions for working with raster datasets and there is substantial overlap between their possibly use cases.
In this book we focus on **terra**, which was developed to be a direct replacement for the older and (in most cases) slower **raster**.
Before learning about the how **terra**'s class system works, which is covered later in the chapter, it is worth understanding similarities and differences between the packages, to help decide which is most appropriate in different situations.
