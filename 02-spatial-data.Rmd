...
However, the transition from **sp** to **sf** will likely take many years.
Many spatial packages may never switch, so it is worth being aware of how to convert `sf` objects into the `Spatial` classes in **sp** using the `as()` function: 
