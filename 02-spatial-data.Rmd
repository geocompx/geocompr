...
Gernally, vector datasets have a high level of precision (but not necessarily accuracy as we will see in \@ref(units)).
On the other hand, the raster data model is good at representing continous phenomena such as elevation or rainfall with the help of grid cell  that break the surface up into a cells of constant size (*resolution*).
...
Finally, use lists for the creation of multilinestrings, (mult-)polygons and geometry collections:
...
The value might be numeric or categoric (\@ref(fig:raster-intro-plot):C).
You can also specify a nodata value in the header of a raster, frequently -9999 (in R we often use `NA`).
<!--
...
-->
Raster maps usually represent continous phenomena such as elevation, temperature, population density or spectral data (\@ref(fig:raster-intro-plot2).
...
```{r raster-intro-plot2, echo=FALSE, fig.cap="Exampled of continuous (left) and categorical (right) raster"}
knitr::include_graphics("figures/raster_intro_plot2.png")
```
