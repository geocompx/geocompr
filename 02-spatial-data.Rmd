...
Other geometry types are described in section \@ref(sf_classes).
```
...
Other than searching for EPSG codes on-line, another quick way to find-out about available CRSs is via the `st_proj_info()` function, which outputs a data frame of available CRSs.
...
The following code will show available CRSs interactively, allowing filtering of ones of interest (try filtering for the Eckert CRSs for example):

```{r, eval=FALSE}
crs_data = st_proj_info()
View(crs_data)
```
...
```{r vector-crs, echo=FALSE, fig.cap="Examples of geographic (left) and projected (right) and coordinate systems for a vector data type."}
...
```{r raster-crs, echo=FALSE, fig.cap="Examples of geographic (left) and projected (right) and coordinate systems for a raster data type"}
