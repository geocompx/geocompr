...
```{r, eval=FALSE}
pkgs = c(
  "raster",        # classes and functions for raster data
  "spData",        # geographic data
  "spDataLarge",   # geographic data
...
```{block2, type='rmdnote'}
On Mac and Linux operating systems a few requirements must be met to install **sf**, as described in the package's [README](https://github.com/r-spatial/sf) file for details.
```
...
In this system London, for example, can be represented by the coordinates `c(-0.1, 51.5)`, meaning it's located at -0.1 degrees east and 55.5 degrees north of the origin in a geographic ('lon/lat') CRS, as illustrated in the left of Figure \@ref(fig:vectorplots)).
...
A `RasterBrick` consists of multiple layers, which typically correspond to a single multispectral satellite file or a single multi-layer object in memory. 
...
Processing of a single mulitilayer file or object is the most effective with `RasterBrick`, while `RasterStack` allows calculations based on many files, or `Raster*` objects, or both.

```{block2 type='rmdnote'}
Ooperations on `RasterBrick` and `RasterStack` objects will typically return a `RasterBrick`.
```
