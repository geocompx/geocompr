...
Note that in the above code chunk we used the `=` sign to create a new object called `world_mini` in the command `world_mini = world[1:2, 1:3]`.
This is called assignment.
Equivalent commands to achieve the same result include `world_mini <- world[1:2, 1:3]` and `assign("world_m", world[1:2, 1:3])`.
...
To change from one style to another we recommend the package [**styler**](https://github.com/r-lib/styler).
```
