...
It is hidden from view for most of the time except when the object is printed but can be can identified and set using the functions `attribute()` and `attr()`, for example `attr(sf_points$geometry, which = "crs")$epsg`.
```
